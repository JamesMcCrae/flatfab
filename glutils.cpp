...
QString GLutils::ColorByIndexStr(const int ind)
{
    ...
    case 0:
        return QString("FF6600"); break;
    case 1:
        return QString("CC0033"); break;
    case 2:
        return QString("FF0099"); break;
    case 3:
        return QString("9900CC"); break;
    case 4:
        return QString("6600FF"); break;
    case 5:
        return QString("0033CC"); break;
    case 6:
        return QString("0099FF"); break;
    case 7:
        return QString("00CC99"); break;
    case 8:
        return QString("00FF66"); break;
    case 9:
        return QString("CCFF00"); break;
    case 10:
    default:
        return QString("CC9900"); break;
    }
...
/*
void GLutils::DrawTNBFrame(const vec3 & o, const TNBFrame & tnb, const float scale)
{
...
void GLutils::DrawArrowFixedLength(const QVector3D & p1, const QVector3D & p2, const float len)
{
...
void GLutils::DrawCylinder(const QVector3D & p1, const QVector3D & p2, const float radius)
{
    ...
    //qDebug() << "DrawCylinder theta_deg" << theta_deg << "axis" << axis << p1 << p2;
...
void GLutils::DrawRing(const QVector3D & p1, const QVector3D & p2, const float inner_rad, const float outer_rad)
{
...
void GLutils::DrawCylinderFixedLength(const QVector3D & p1, const QVector3D & p2, const float radius, const float len)
{
...
vec2 GLutils::ProjectPoint(const vec3 & p)
{
    ...
    gluProject(p[0], p[1], p[2], model_view, projection, viewport, &winx, &winy, &winz);
...
QVector3D GLutils::ProjectPoint(const QVector3D & p)
{
    ...
    gluProject(p.x(), p.y(), p.z(), model_view, projection, viewport, &winx, &winy, &winz);
...
void GLutils::UnProjectPoint(const QVector2D & v, const float depth_value, QVector3D & unproj_pt)
{
    ...
    gluUnProject(v.x(), v.y(), depth_value, model_view, projection, viewport, &objx, &objy, &objz);
...
void GLutils::DisableBlending()
{
    glDisable(GL_BLEND);
}

void GLutils::ReadPixelColor(const int x, const int y, unsigned char & r, unsigned char & g, unsigned char & b)
{
...
void GLutils::ReadPixelColor_FrontBuffer(const int x, const int y, unsigned char & r, unsigned char & g, unsigned char & b)
{
...
void GLutils::ReadPixelColor_BackBuffer(const int x, const int y, unsigned char & r, unsigned char & g, unsigned char & b)
{
...
void GLutils::PixelColorToIndex(const unsigned char r, const unsigned char g, const unsigned char b, int & index)
{
...
void GLutils::IndexToPixelColor(const int index, unsigned char & r, unsigned char & g, unsigned char & b)
{
...
void GLutils::GetViewport(int v[4])
{
...
QVector3D GLutils::RotateVector(const QVector3D & v, const QVector3D & axis, const float angle_radians)
{
    ...
    return QVector3D(QVector3D::dotProduct(v, R1), QVector3D::dotProduct(v, R2), QVector3D::dotProduct(v, R3));
...
void GLutils::GetBoundingBox(const QVector <QVector3D> & vs, QVector3D & bbox_min, QVector3D & bbox_max)
{
...
float GLutils::AngleBetweenDeg(const QVector3D & v1, const QVector3D & v2)
{
    ...
    const float dotprod = QVector3D::dotProduct(v1.normalized(), v2.normalized());
...
float GLutils::SignedAngleBetweenRad(const QVector3D & v1, const QVector3D & v2, const QVector3D & axis)
{

    const float dot = QVector3D::dotProduct(v1, v2);
    //det = x1*y2*zn + x2*yn*z1 + xn*y1*z2 - z1*y2*xn - z2*yn*x1 - zn*y1*x2
    const float det = v1.x() * v2.y() * axis.z() +
            v2.x() * axis.y() * v1.z() +
            axis.x() * v1.y() * v2.z() -
            v1.z() * v2.y() * axis.x() -
            v2.z() * axis.y() * v1.x() -
...
bool GLutils::LineLineIntersection(const QVector2D & p1, const QVector2D & p2, const QVector2D & p3, const QVector2D & p4, QVector2D & intersect)
{

    //http://paulbourke.net/geometry/pointlineplane/

    const float u_denom = (p4.y() - p3.y()) *  (p2.x() - p1.x()) - (p4.x() - p3.x()) * (p2.y() - p1.y());

    if (fabs(u_denom) < 0.0001f) {
        return false;
    }

    const float ua_num = (p4.x() - p3.x()) * (p1.y() - p3.y()) - (p4.y() - p3.y()) * (p1.x() - p3.x());
    const float ub_num = (p2.x() - p1.x()) * (p1.y() - p3.y()) - (p2.y() - p1.y()) * (p1.x() - p3.x());
...
bool GLutils::LineRayIntersection(const QVector2D & p1, const QVector2D & p2, const QVector2D & lp, const QVector2D & ld, QVector2D & intersect)
{

    const float u_denom = ld.y() *  (p2.x() - p1.x()) - ld.x() * (p2.y() - p1.y());

    if (fabs(u_denom) < 0.0001f) {
        return false;
    }

    const float ua_num = ld.x() * (p1.y() - lp.y()) - ld.y() * (p1.x() - lp.x());
...
bool GLutils::LineBoxIntersection(const QVector2D & p1, const QVector2D & p2, const QVector2D & lp, const QVector2D & ld)
{
...
bool GLutils::LinePlaneIntersection(const QVector3D & p0, const QVector3D & n, const QVector3D & l0, const QVector3D & l1, QVector3D & intersect)
{
...
bool GLutils::LineSegmentPlaneIntersection(const QVector3D & p0, const QVector3D & n, const QVector3D & p1, const QVector3D & p2, QVector3D & intersect)
{
...
float GLutils::PointLineSignedDistance(const QVector2D & l1, const QVector2D & l2, const QVector2D & p)
{
...
float GLutils::PointLineSegmentDistance(const QVector3D & p0, const QVector3D & p1, const QVector3D & p2)
{
    ...
    if (t >= 1.0f) {
        ...
        //subtract the projection from p1p0, use that length (its the orthogonal length from p0 to line)
...
float GLutils::PointLineSegmentDistance(const QVector3D & p0, const QVector3D & p1, const QVector3D & p2, QVector3D & best_pt)
{
...
bool GLutils::PlanePlaneIntersection(const QVector3D & n1, const QVector3D & p1, const QVector3D & n2, const QVector3D & p2, QVector3D & lp, QVector3D & ld)
{
...
void GLutils::ConvexHull_GiftWrapping(const QList <QVector3D> & pts, QList <int> & hull)
{
    ...
    for (int i=0; i<pts.size(); ++i) {
        ...
        //we now select point pi such that all other points are "to the right" of the line between p0 and p*
        int endpoint = 0;

        for (int j=0; j<pts.size(); ++j) {

            if (endpoint == hull.last()) { //we need two distinct points to form a line
                endpoint = j;
            }
            else { //endpoint and point_on_hull form a line
                //if line they form is such that pts[j] is "left" of it
                //then pts[j] is our new endpoint and candidate for next hull point
...
bool GLutils::ConvexHull_PointInside(const QList <QVector3D> & hull, const QVector3D & p)
{

    if (hull.size() == 2) {


        const float cross_prod = QVector3D::crossProduct(hull[1] - p, hull[0] - p).y();
...
QVector3D GLutils::GetVectorNewBasis(const QVector3D & old_x, const QVector3D & old_y, const QVector3D & old_z, const QVector3D & old_p,
...
void GLutils::SortPointsAlongDirection2D(const QVector2D & dir, QList <QVector2D> & isecs)
{
...
void GLutils::SortPointsAlongDirection3D(const QVector3D & dir, QList <QVector3D> & pts)
{
...
void GLutils::SortPointsAlongDirection3DExtra(const QVector3D dir, QList <QVector3D> & isecs, QList <bool> & isecs_which)
{
...
void GLutils::GetClosestPairOfPoints(const QList <QVector3D> & pts1, const QList <QVector3D> & pts2, int & ind1, int & ind2)
{
...
void GLutils::GetModelviewMatrixFromVec(const QVector3D & v, const QVector3D & p, float modelview[16])
{
...
void GLutils::GetSortedIntersectionPoints(const QList <QVector2D> & pts, const QVector2D ray_dir, const QVector2D ray_p, QList <QVector2D> & sorted_isecs)
{
    ...
    if (sorted_isecs.size() >= 2) {
        SortPointsAlongDirection2D(sorted_isecs.last() - sorted_isecs.first(), sorted_isecs);
    }

...
void BezierFit::FitCurve(const QList <QVector2D> & d, const double error, QList <QVector2D> & fit_curve)
{

    //fit_curve.clear();

    //const QVector2D tHat1 = ComputeLeftTangent(d, 0); /*  Unit tangent vectors at endpoints */
    //const QVector2D tHat2 = ComputeRightTangent(d, d.size() - 1);
    //FitCubic(d, 0, d.size() - 1, tHat1, tHat2, error, fit_curve);
...
void BezierFit::FitCubic(const QList <QVector2D> & d, const int first, const int last, const QVector2D & tHat1, const QVector2D & tHat2, const double error, QList <QVector2D> & fit_curve)

{
    ...
    if (nPts == 2) {
        double dist = (d[last] - d[first]).length(); //double dist = V2DistanceBetween2Points(&d[last], &d[first]) / 3.0;

        bezCurve.resize(4); //bezCurve = (Point2 *)malloc(4 * sizeof(Point2));
        bezCurve[0] = d[first];
        bezCurve[3] = d[last];
        bezCurve[1] = bezCurve[0] + (tHat1 * dist); //V2Add(&bezCurve[0], V2Scale(&tHat1, dist), &bezCurve[1]);
        bezCurve[2] = bezCurve[3] + (tHat2 * dist); //V2Add(&bezCurve[3], V2Scale(&tHat2, dist), &bezCurve[2]);
    ...
    if (maxError < iterationError) {
        for (int i = 0; i < maxIterations; i++) {
            ...
            if (maxError < error) {
                ...
                //qDebug() << "BezierFit::FitCubic - line 134" << first << last;
                fit_curve.push_back(bezCurve[0]);
                fit_curve.push_back(bezCurve[1]);
                fit_curve.push_back(bezCurve[2]);
                fit_curve.push_back(bezCurve[3]);
 ...
 *
 */
void BezierFit::GenerateBezier(const QList <QVector2D> & d, const int first, const int last, const QVector <double> & uPrime, const QVector2D tHat1, const QVector2D tHat2, QVector <QVector2D> & bezCurve)
//    Point2	*d;			//  Array of digitized points
...
//    Vector2	tHat1, tHat2;	//  Unit tangents at endpoints
{
    ...
    for (int i = 0; i < nPts; i++) {
        C[0][0] += QVector2D::dotProduct(A[i][0], A[i][0]); //V2Dot(&A[i][0], &A[i][0]);
        C[0][1] += QVector2D::dotProduct(A[i][0], A[i][1]); //V2Dot(&A[i][0], &A[i][1]);
        /*					C[1][0] += V2Dot(&A[i][0], &A[i][1]);*/
        C[1][0] = C[0][1];
        C[1][1] += QVector2D::dotProduct(A[i][1], A[i][1]); //V2Dot(&A[i][1], &A[i][1]);
    ...
    double segLength = (d[last] - d[first]).length(); //V2DistanceBetween2Points(&d[last], &d[first]);
    ...
    bezCurve[0] = d[first];
    bezCurve[3] = d[last];
    bezCurve[1] = bezCurve[0] + (tHat1 * alpha_l); //V2Add(&bezCurve[0], V2Scale(&tHat1, alpha_l), &bezCurve[1]);
    bezCurve[2] = bezCurve[3] + (tHat2 * alpha_r); //V2Add(&bezCurve[3], V2Scale(&tHat2, alpha_r), &bezCurve[2]);
    return; //return (bezCurve);
}
 ...
 *
 */
void BezierFit::Reparameterize(const QList <QVector2D> & d, const int first, const int last, const QVector <double> & u, const QVector <QVector2D> & bezCurve, QVector <double> & uPrime)
//    Point2	*d;			/*  Array of digitized points	*/
//    int		first, last;		/*  Indices defining region	*/
 ...
 */
double BezierFit::NewtonRaphsonRootFind(const QVector <QVector2D> & Q, const QVector2D & P, const double u)
...
//    double 		u;		/*  Parameter value for "P"	*/
{
    ...
    for (int i = 0; i <= 2; i++) {
        ...
        Q1[i] = QVector2D((Q[i+1].x() - Q[i].x()) * 3.0, (Q[i+1].y() - Q[i].y()) * 3.0);
    }
    ...
    for (int i = 0; i <= 1; i++) {
        ...
        Q2[i] = QVector2D((Q1[i+1].x() - Q1[i].x()) * 2.0, (Q1[i+1].y() - Q1[i].y()) * 2.0);
    }
 ...
 *
 */
QVector2D BezierFit::BezierII(const int degree, const QVector <QVector2D> & V, const double t)
...
//    double 	t;		/* Parametric value to find point for	*/
{
    ...
    /* Copy array	*/
    Vtemp.resize(degree+1); //Vtemp = (Point2 *)malloc((unsigned)((degree+1) * sizeof (Point2)));
    ...
    for (int i = 1; i <= degree; i++) {
        for (int j = 0; j <= degree-i; j++) {
            ...
            Vtemp[j] = QVector2D((1.0 - t) * Vtemp[j].x() + t * Vtemp[j+1].x(),
...
double BezierFit::B3(const double u)
{
    return (u * u * u);
}
 ...
 */
QVector2D BezierFit::ComputeLeftTangent(const QList <QVector2D> & d, const int end)
...
QVector2D BezierFit::ComputeRightTangent(const QList <QVector2D> & d, const int end)
//    Point2	*d;			//  Digitized points
//    int		end;		// Index to "left" end of region
{
    QVector2D	tHat2;
...
QVector2D BezierFit::ComputeCenterTangent(const QList <QVector2D> & d, const int center)
//    Point2	*d;			//  Digitized points
 ...
 */
void BezierFit::ChordLengthParameterize(const QList <QVector2D> & d, const int first, const int last, QVector <double> & u)
//    Point2	*d;			// Array of digitized points
//    int		first, last;		//  Indices defining region
{
    int		i;
    ...
    for (i = first+1; i <= last; i++) {
        u[i-first] = u[i-first-1] + (d[i] - d[i-1]).length(); //V2DistanceBetween2Points(&d[i], &d[i-1]);
    }
 ...
 *	to fitted curve.
*/
double BezierFit::ComputeMaxError(const QList <QVector2D> & d, const int first, const int last, const QVector <QVector2D> & bezCurve, const QVector <double> & u, int & splitPoint)
//    Point2	*d;			//  Array of digitized points
//    int		first, last;		//  Indices defining region

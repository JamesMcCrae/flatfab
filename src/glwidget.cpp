...
CPhysics &GLWidget::GetPhysics() { return physics; }
...
QWidget *GLWidget::GetEditWidget()
{
    ...
    grid_staple_label =
        new QLabel(QString::number(GetGenerateGridStapleSize()));
    linear_spacing_label =
        new QLabel(QString::number(GetGenerateLinearSpacing()));
    slices_spacing_label =
        new QLabel(QString::number(GetGenerateSlicesSpacing()));
    num_blend_sections_label =
        new QLabel(QString::number(GetGenerateBlendSections()));
    num_revolve_sections_label =
        new QLabel(QString::number(GetGenerateRevolveSections()));
    branching_scalechild_label =
        new QLabel(QString::number(GetGenerateBranchingScaleChild()));
    radial_sectors_label =
        new QLabel(QString::number(GetGenerateRadialSectors()));
    ...
    deleteButton->setStyleSheet(
        "QPushButton{ background-color: #40FA42; color:black}");  // for any
                                                                  // delete
                                                                  // button
    ...
    connect(radial_sectors_slider, SIGNAL(valueChanged(int)), this,
            SLOT(SetGenerateRadialSectors(int)));
...
QWidget *GLWidget::GetGenerateWidget()
{
    ...
    connect(num_blend_sections_slider, SIGNAL(valueChanged(int)), this,
            SLOT(SetGenerateBlendSections(int)));
    ...
    branching_scalechild_slider->setValue(
        this->GetGenerateBranchingScaleChild() * 100);
    connect(branching_scalechild_slider, SIGNAL(valueChanged(int)), this,
            SLOT(SetGenerateBranchingScaleChild(int)));
    ...
    connect(grid_sizex_slider, SIGNAL(valueChanged(int)), this,
            SLOT(SetGenerateGridSizeX(int)));
    ...
    connect(grid_sizey_slider, SIGNAL(valueChanged(int)), this,
            SLOT(SetGenerateGridSizeY(int)));
    ...
    connect(grid_staplesize_slider, SIGNAL(valueChanged(int)), this,
            SLOT(SetGenerateGridStapleSize(int)));
    ...
    connect(linear_spacing_slider, SIGNAL(valueChanged(int)), this,
            SLOT(SetGenerateLinearSpacing(int)));

    QCheckBox *linear_scalex_checkbox = new QCheckBox("Scale X");
    linear_scalex_checkbox->setChecked(GetGenerateLinearScaleX());
    connect(linear_scalex_checkbox, SIGNAL(toggled(bool)), this,
            SLOT(SetGenerateLinearScaleX(bool)));

    QCheckBox *linear_scaley_checkbox = new QCheckBox("Scale Y");
    linear_scaley_checkbox->setChecked(GetGenerateLinearScaleY());
    connect(linear_scaley_checkbox, SIGNAL(toggled(bool)), this,
            SLOT(SetGenerateLinearScaleY(bool)));
    ...
    connect(num_revolve_sections_slider, SIGNAL(valueChanged(int)), this,
            SLOT(SetGenerateRevolveSections(int)));
    ...
    connect(slices_spacing_slider, SIGNAL(sliderReleased()), this,
            SLOT(UpdateGenerateSlicesSpacing()));
    connect(slices_spacing_slider, SIGNAL(valueChanged(int)), this,
            SLOT(SetGenerateSlicesSpacing(int)));

    QCheckBox *slices_x_checkbox = new QCheckBox("X");
    slices_x_checkbox->setChecked(GetGenerateSlicesX());
    connect(slices_x_checkbox, SIGNAL(toggled(bool)), this,
            SLOT(SetGenerateSlicesX(bool)));

    QCheckBox *slices_y_checkbox = new QCheckBox("Y");
    slices_y_checkbox->setChecked(GetGenerateSlicesY());
    connect(slices_y_checkbox, SIGNAL(toggled(bool)), this,
            SLOT(SetGenerateSlicesY(bool)));

    QCheckBox *slices_z_checkbox = new QCheckBox("Z");
    slices_z_checkbox->setChecked(GetGenerateSlicesZ());
    connect(slices_z_checkbox, SIGNAL(toggled(bool)), this,
            SLOT(SetGenerateSlicesZ(bool)));
    ...
    connect(surfaceFacetsButton, SIGNAL(clicked()), this,
            SLOT(DoGenerateSurfaceFacets()));
    ...
    connect(surfacefacets_teeth_checkbox, SIGNAL(toggled(bool)), this,
            SLOT(SetGenerateSurfaceFacetsTeeth(bool)));

    QGroupBox *surfacefacet_groupbox =
        new QGroupBox(tr("Surface Facets Operation"));
...
QWidget *GLWidget::GetGuidesWidget()
{
    ...
    connect(dimensioningToolButton, SIGNAL(clicked()), this,
    ...
    connect(calibration_slider, SIGNAL(valueChanged(int)), this,
            SLOT(SetCalibrationFactor(int)));
    ...
    connect(quality_slider, SIGNAL(valueChanged(int)), this,
            SLOT(SetQualitySamples(int)));

    thick_label =
        new QLabel(QString::number(thick_spinbox->value()) + QString(" units"));
    calibration_label =
        new QLabel(QString::number(calibration_slider->value()) + QString("%"));
    ...
    connect(magnetic_cuts_checkbox, SIGNAL(toggled(bool)), this,
            SLOT(SetDoMagneticCuts(bool)));
    ...
    connect(loadImageButton, SIGNAL(clicked()), this,
            SLOT(LoadTemplateImage()));
    ...
    connect(imagex_spinbox, SIGNAL(valueChanged(int)), this,
            SLOT(SetTemplateImageX(int)));
    ...
    connect(imagey_spinbox, SIGNAL(valueChanged(int)), this,
            SLOT(SetTemplateImageY(int)));
    ...
    connect(imagerotate_spinbox, SIGNAL(valueChanged(int)), this,
            SLOT(SetTemplateImageRotate(int)));
    ...
    connect(imagescale_spinbox, SIGNAL(valueChanged(int)), this,
            SLOT(SetTemplateImageScale(int)));

    QCheckBox *imageflipx_checkbox = new QCheckBox("Horizontal Flip");
    imageflipx_checkbox->setChecked(template_flipx);
    connect(imageflipx_checkbox, SIGNAL(toggled(bool)), this,
            SLOT(SetTemplateImageFlipX(bool)));
    ...
    connect(toggleTemplatesButton, SIGNAL(clicked()), this,
            SLOT(ToggleDrawTemplates()));
...
QWidget *GLWidget::GetPhysicsWidget()
{
    ...
    connect(testPhysicsButton, SIGNAL(clicked()), this,
            SLOT(ToggleDoPhysicsTest()));
    ...
    connect(weightButton2, SIGNAL(clicked()), this,
            SLOT(DoPhysicsRemoveWeights()));
    weightButton2->setStyleSheet(
        "QPushButton{ background-color: #40FA42; color:black}");  // for any
                                                                  // delete
                                                                  // button
    ...
    connect(new_weight_spinbox, SIGNAL(valueChanged(double)), this,
            SLOT(SetPhysicsNewWeightMass(double)));

    new_weight_label = new QLabel(QString::number(new_weight_spinbox->value()) +
                                  QString(" kilograms"));
    ...
    connect(density_spinbox, SIGNAL(valueChanged(double)), this,
            SLOT(SetPhysicsMaterialDensity(double)));
    ...
    connect(stress_spinbox, SIGNAL(valueChanged(double)), this,
            SLOT(SetPhysicsMaximumStress(double)));
...
QWidget *GLWidget::GetViewWidget()
{
    ...
    connect(rotation_slider, SIGNAL(valueChanged(int)), this,
            SLOT(SetRotationDuration(int)));
    ...
    connect(rotation_angle_slider, SIGNAL(valueChanged(int)), this,
            SLOT(SetRotationAngle(int)));

    rotate_label = new QLabel(QString::number(rotation_slider->value() * 100) +
                              QString(" ms"));
    rotate_angle_label = new QLabel(
        QString::number(rotation_angle_slider->value()) + QString(" degrees"));
...
void GLWidget::paintGL()
{
    ...
    if (sections_disp_list > 0) {
        ...
        for (int i = 0; i < sections.size(); ++i) {
            if (current_tool_state == TOOLSTATE_GENERATE) {
                if (!(i == selected ||
                      (generate_selections.size() > 0 &&
                       i == generate_selections[0]) ||
                      (generate_selections.size() > 1 &&
                       i == generate_selections[1]) ||
                      (generate_selections.size() == 3 &&
                       i == generate_selections[2]))) {
                    DrawSection(i);
                }
            } else if (i != selected) {
                DrawSection(i);
    ...
    if (do_stability_test) {
        PlanarSection::DrawConvexHull(sections_convex_hull,
                                      centre_of_mass_in_hull);
    }
    ...
    if (do_show_templates) {
        if (state == STATE_SLOT ||  // 2.
            state == STATE_CAM_TRANSLATE || state == STATE_DEADZONE ||
            state == STATE_CURVE || state == STATE_PEN_POINT ||
            state == STATE_PEN_DRAG || state == STATE_MANIP_CTRLPOINT ||  // 3.
            sections.empty()) {                                           // 1.
    ...
    */
    ...
    if (state == STATE_PEN_POINT || state == STATE_PEN_DRAG)
        active_section.DrawCurveControlPointsHandleStyle(cam.CamWidth(),
                                                         cam.Eye());
    ...
    if (do_local_symmetry) {
        if (sections.empty()) {
            ...
            glEnd();
            glLineWidth(1);
        } else if (state == STATE_CURVE || state == STATE_RESKETCH_CURVE ||
                   state == STATE_DEADZONE || state == STATE_PEN_POINT ||
                   state == STATE_PEN_DRAG) {
            QVector3D dir =
                (slot_start_projected - slot_end_projected).normalized();
            float factor = (20.0 * 600.0 / height()) / cam.CamWidth();
            glEnable(GL_BLEND);
            glLineWidth(3);
            glColor4f(0.3f, 0.5f, 0.8f, 0.5f);
            glBegin(GL_LINES);
            glVertex2f(slot_start_projected.x() + factor * dir.x(),
                       slot_start_projected.y() + factor * dir.y());
            glVertex2f(slot_start_projected.x() + 10000 * dir.x(),
                       slot_start_projected.y() + 10000 * dir.y());
            glVertex2f(slot_end_projected.x() - factor * dir.x(),
                       slot_end_projected.y() - factor * dir.y());
            glVertex2f(slot_end_projected.x() - 10000 * dir.x(),
                       slot_end_projected.y() - 10000 * dir.y());
            glEnd();
            glLineWidth(1);
        }
    }
...
{
    if (current_tool_state != TOOLSTATE_GENERATE && state != STATE_PEN_POINT &&
        state != STATE_PEN_DRAG) {
        return;
    }
    ...
    if (current_tool_state == TOOLSTATE_GENERATE) {
        switch (gen_state) {
            case GENSTATE_LINEAR:
                ...
                text =
                    "Instructions\n\n 1) Select the base section (right-click "
                    "away from sections to reset selection)\n 2) Select a "
                    "section that intersects the base\n 3) Press Enter to "
            ...
            case GENSTATE_BLEND:
                ...
                text =
                    "Instructions\n\n 1) Select the base section (right-click "
                    "away from sections to reset selection)\n 2) Select two "
                    "sections that intersect the base\n 3) Press Enter to "
                    "accept or Escape to cancel";
                break;
            ...
            case GENSTATE_SLICES:
                ...
                text =
                    "Instructions\n\n 1) A template obj must be loaded - This "
                    "can be done in the Guides side panel\n 2) Press Enter to "
                    "accept or Escape to cancel";
                break;
            ...
            case GENSTATE_BRANCH:  // branching isn't active yet
                ...
                text =
                    "Instructions\n\n 1) Set the root slot by clicking and "
                    "dragging on a section\n 2) Add branches 3) Press Enter to "
                    "accept or Escape to cancel";
                break;
        ...
        title = "Pen Mode";
        text =
            "Left-click and drag to create a new point and handles\nPress "
    ...
    painter.drawText(border, border + 20, rect.width(), rect.height(),
                     Qt::AlignLeft | Qt::TextWordWrap, text);
}
...
void GLWidget::mousePressEvent(QMouseEvent *event)
{
    ...
    */
    ...
    if (event->button() == Qt::LeftButton) {
        if (!(pen_mode && state == STATE_PEN_POINT) &&
            ...
            switch (state) {
                case STATE_TRANSFORM_WIDGET:
                    ...
                    if (IsSectionSelected()) {
                        ...
                        if (index > 0) {
                            transform_widget.SetState(
                                TransformWidgetState(index));
                ...
                case STATE_PEN_POINT:
                    ...
                    active_section.SelectCtrlPoint(
                        active_section.GetCurve(0).Points().size() -
                        3);  // the last two points in the curve attach to the
                             // first point
                    break;
                ...
                default:
                    ...
                    if (sections.empty()) {
                        ...
                        if (pen_mode) {
                            ...
                            active_section.SelectCtrlPoint(
                                active_section.GetCurve(0).Points().size() -
                                3);  // the last two points in the curve attach
                                     // to the first point
                        } else {
                            state = STATE_CURVE;
                        }
                        ...
                        const int section_clicked =
                            PickSection(mouse_pos, false);

                        if (section_clicked >= 0 &&
                            section_clicked < sections.size()) {
                            QVector3D p;
                            sections[section_clicked].MouseRayIntersect(
                                mouse_pos, p);

                            // If selectiing sections for the generate tools
                            if (current_tool_state == TOOLSTATE_GENERATE &&
                                gen_state != GENSTATE_BRANCH) {
                                if (generate_selections.size() <
                                        selections_per_gen_type[gen_state] ||
                                    gen_state == GENSTATE_GRID) {
                                    if (!generate_selections.contains(
                                            section_clicked)) {
                                        if (gen_state == GENSTATE_GRID &&
                                            generate_selections.size() == 1) {
                                            generate_selections[0] =
                                                section_clicked;  // this allows
                                                                  // the user to
                                                                  // switch
                                                                  // sections
                                                                  // during grid
                            ...
                            } else if (current_tool_state ==
                                       TOOLSTATE_DEFAULT) {
                                active_section = PlanarSection();
                                SetupPlanarSection(active_section);
                                active_section.SetP(p);
                                active_section.SetN(
                                    sections[section_clicked].T());
                                active_section.SetT(
                                    sections[section_clicked].B());
                                active_section.SetB(
                                    sections[section_clicked].N());
        ...
        if (current_tool_state == TOOLSTATE_GENERATE) {
            ...
            // bugfix: only add the selection index to the list if there is an
            // actual selection (new_select >= 0)
            if (new_select < 0) {
                generate_selections.clear();
                ShowGenerate();
            } else if (generate_selections.size() <
                           selections_per_gen_type[gen_state] ||
                       gen_state == GENSTATE_GRID) {
                if (!generate_selections.contains(new_select)) {
                    if (gen_state == GENSTATE_GRID &&
                        generate_selections.size() == 1) {
                        generate_selections[0] =
                            new_select;  // this allows the user to switch
                                         // sections during grid generation
                    } else {
    ...
    if (event->buttons() & Qt::LeftButton) {
        switch (state) {
                ...
                if (IsSectionSelected()) {
                    ...
                    switch (transform_widget.GetState()) {
                        case TRANS_X: {
                            sections[selected].MouseRayIntersect(mouse_pos,
                                                                 cursor_p);
                            sections[selected].MouseRayIntersect(
                                anchor_point, initial_cursor_p);
                            const float dot_prod = QVector3D::dotProduct(
                                t, (cursor_p - initial_cursor_p));
                        ...
                        case TRANS_Y: {
                            ...
                            ps.MouseRayIntersect(anchor_point,
                                                 initial_cursor_p);
                            float dot_prod = QVector3D::dotProduct(
                                n, (cursor_p - initial_cursor_p));
                        ...
                        case ROT_X: {
                            ...
                            QVector3D new_n = GLutils::RotateVector(
                                n, t,
                                GLutils::SignedAngleBetweenRad(v1, v2, t));

                            sections[selected].SetN(new_n);
                            sections[selected].SetB(
                                QVector3D::crossProduct(new_n, t).normalized());
                        } break;
                        case ROT_Y: {
                            sections[selected].MouseRayIntersect(mouse_pos,
                                                                 cursor_p);
                            ...
                            QVector3D v1 = (initial_cursor_p - p).normalized();
                            QVector3D v2 = (cursor_p - p).normalized();
                            QVector3D new_b = GLutils::RotateVector(
                                b, n,
                                GLutils::SignedAngleBetweenRad(v1, v2, n));

                            sections[selected].SetB(new_b);
                            sections[selected].SetT(
                                QVector3D::crossProduct(new_b, n).normalized());
                        } break;
                        case ROT_Z: {
                            PlanarSection ps;
                            ps.SetP(p);
                            ...
                            QVector3D v1 = (initial_cursor_p - p).normalized();
                            QVector3D v2 = (cursor_p - p).normalized();
                            QVector3D new_t = GLutils::RotateVector(
                                t, b,
                                GLutils::SignedAngleBetweenRad(v1, v2, b));

                            sections[selected].SetT(new_t);
                            sections[selected].SetN(
                                QVector3D::crossProduct(new_t, b).normalized());
                        } break;
            ...
            case STATE_SLOT:
                ...
                if (PickSection(mouse_pos, true) == selected) {
                    ...
                    const QVector3D new_t =
                        (-sections[selected].N()).normalized();
                    const QVector3D new_b =
                        (p - active_section.P()).normalized();
                    const QVector3D new_n =
                        QVector3D::crossProduct(new_t, new_b).normalized();
                    ...
                    // we need to initiate the rotation,  but keep the vertex
                    // position slot_end constant in screen space...

                    sections[selected].MouseRayIntersect(mouse_pos, slot_end);

                    QVector3D v;
                    sections[selected].MouseRayIntersect(
                        QVector2D(float(this->width()) / 2.0f,
                                  float(this->height() / 2.0f)),
                        v);

                    QVector3D vs[4];
                    vs[0] = GLutils::RotateVector(
                        active_section.T(), active_section.B(),
                        cam_animate_angle * 3.14159f / 180.0f);
                    ...
                    int best_index = 0;
                    float best_val =
                        FLT_MAX;  // best val is a MINIMUM dot product

                    // among 4 choices, pick the one where of the downward
                    // viewing ones, the dot product with current view is lowest
                    for (int i = 0; i < 4; ++i) {
                        if (vs[i].y() > 0.0f) {
                            continue;
                        }
            ...
            case STATE_DEADZONE:
                ...
                if (!pen_mode &&
                    active_section.MouseOutsideDeadzone(
                        mouse_pos, slot_start, slot_end, deadzone_radius)) {
                    state = STATE_CURVE;
                    active_section.SketchSetEditing(true);
                }
            ...
            case STATE_RESKETCH_CURVE:
                ...
                if (IsSectionSelected()) {
                    ...
                    if (do_show_templates && !template_cut.empty()) {
                        sections[selected].SnapSketchToTemplateCut(
                            template_cut, template_cut_snap_distance_3d);
                        sections[selected].Update(
                            0, section_error_tolerance_template);
                    } else {
            ...
            case STATE_ADD_HOLE:
                ...
                if (IsSectionSelected()) {
                    sections[selected].AddMouseRayIntersect(
                        sections[selected].GetNumCurves() - 1, mouse_pos);
                    sections[selected].Update(
                        sections[selected].GetNumCurves() - 1,
                        section_error_tolerance);
                }
            ...
            case STATE_CURVE:
                ...
                // if we have a template, do snapping
                if (do_show_templates && !template_cut.empty()) {
                    active_section.SnapSketchToTemplateCut(
                        template_cut, template_cut_snap_distance_3d);
        ...
        if (state == STATE_PEN_DRAG) {
            ...
            if (state == STATE_MANIP_WEIGHT) {
                ...
                sections[selected].MoveCtrlPointMouseRayIntersect(mouse_pos,
                                                                  !ctrl_held);
                sections[selected].UpdateCurveTrisSlab();
            }
    ...
    if (event->button() == Qt::LeftButton) {
        switch (state) {
            ...
            case STATE_DEADZONE:
                ...
                // if the gesture wasn't completed, remove the section being
                // added sections.removeLast(); active_section = null_section;
                if (pen_mode) {
                    state = STATE_PEN_POINT;
                } else if (selected != -1) {
                    SetSelected(-1);
            ...
            case STATE_RESKETCH_CURVE:
                ...
                if (!template_cut.empty()) {
                    sections[selected].Update(0,
                                              section_error_tolerance_template);
                } else {
                    sections[selected].Update(0, section_error_tolerance);
                }
            ...
            case STATE_CURVE:
                ...
                if (active_section.SketchNumPoints() >= 5) {
                    ...
                    if (do_local_symmetry &&
                        !active_section_symmetry.SliceTriangles().empty()) {
                        sections.push_back(active_section_symmetry);
                        UpdateAllTests();
                    } else if (!do_local_symmetry &&
                               !active_section.SliceTriangles().empty()) {
                        // for the added curve, we now update our physical tests
                        // (TODO: checkbox/boolean to disable this)
            ...
            {
                bool ok;
                double new_length = QInputDialog::getDouble(
                    this, QString("Dimensioning Tool"),
                    QString("Specify a length for the defined line segment:"),
                    5.0, 0.1, 100.0, 1, &ok, Qt::Dialog);
                if (ok) {
                    // scale the model by the ratio of the defined length and
                    // the entered value
                    AddToUndoList(OP_MANIP_DIMENSIONING_TOOL);

                    const double scale_val =
                        new_length /
                        (dimensiontool_end - dimensiontool_start).length();
...
void GLWidget::keyReleaseEvent(QKeyEvent *event)
{
    switch (event->key()) {
        case Qt::Key_Minus:
            ...
            if (state == STATE_PEN_POINT) {
                active_section.DeleteCtrlPoint(
                    0, active_section.GetCurve(0).GetNumControlPoints() - 3);
                active_section.UpdateCurveTrisSlab();
        ...
        case Qt::Key_Plus:
            ...
            if (IsSectionSelected()) {
                if (sections[selected].IsCtrlPointSelected()) {
                    ...
                    UpdateCamera();
                    sections[selected].AddWeightAtMousePos(
                        mouse_pos, physics_new_weight_mass);
                }
...
void GLWidget::DrawSection(const int i)
{
    ...
    if (do_physics_test) sections[i].DrawWeights();
...
int GLWidget::PickSection(const QVector2D &mouse_pos,
                          const bool only_test_selected)
{
    glEnable(GL_DEPTH_TEST);
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
...
void GLWidget::SetUnitsToInches() { SetMetresPerUnit(0.0254); }

void GLWidget::SetUnitsToCentimetres() { SetMetresPerUnit(0.01); }
...
void GLWidget::SetViewPart()
{
    if (!sections.empty() && IsSectionSelected()) {
        ...
        const float desired_cam_width =
            qMax(max_v.x() - min_v.x(), max_v.y() - min_v.y()) * 1.2f;
        ...
        if (sections[selected].N().y() > 0.0f) {
            cam.SetEye(centre_3d +
                       sections[selected].N() * cam_lookat_distance);
        } else {
            cam.SetEye(centre_3d -
                       sections[selected].N() * cam_lookat_distance);
        }
...
bool GLWidget::GetPenModeOn() { return pen_mode; }
...
bool GLWidget::GetDoPhysicsTest() { return do_physics_test; }

bool GLWidget::GetDoConnectedTest() { return do_connected_test; }

bool GLWidget::GetShowTNBFrames() { return do_show_tnb_frames; }

bool GLWidget::GetShowShadow() { return do_show_shadow; }

bool GLWidget::GetShowTemplates() { return do_show_templates; }

float GLWidget::GetGenerateGridSizeX() { return generate_grid_sizex; }

float GLWidget::GetGenerateGridSizeY() { return generate_grid_sizey; }

float GLWidget::GetGenerateGridStapleSize() { return generate_grid_staplesize; }

float GLWidget::GetGenerateLinearSpacing() { return generate_linear_spacing; }

bool GLWidget::GetGenerateLinearScaleX() { return generate_linear_scalex; }

bool GLWidget::GetGenerateLinearScaleY() { return generate_linear_scaley; }

float GLWidget::GetGenerateSlicesSpacing() { return generate_slices_spacing; }

bool GLWidget::GetGenerateSlicesX() { return generate_slices_x; }

bool GLWidget::GetGenerateSlicesY() { return generate_slices_y; }

bool GLWidget::GetGenerateSlicesZ() { return generate_slices_z; }

int GLWidget::GetGenerateBlendSections() { return generate_blend_sections; }

int GLWidget::GetGenerateRevolveSections() { return generate_revolve_sections; }
...
int GLWidget::GetGenerateRadialSectors() { return generate_radial_sectors; }
...
double GLWidget::GetPhysicsNewWeightMass() { return physics_new_weight_mass; }
...
double GLWidget::GetPhysicsMaximumStress() { return physics_max_stress; }
...
QString GLWidget::GetOpenFilename() const { return open_filename; }
...
void GLWidget::ToggleDoPhysicsTest()
{
    ...
    if (do_physics_test) DoPhysicsTest();
...
void GLWidget::SetShowTNBFrames(const bool b) { do_show_tnb_frames = b; }
...
void GLWidget::SetShowTemplates(const bool b) { do_show_templates = b; }
...
void GLWidget::SetCalibrationFactor(const int i)
{
    ...
    calibration_label->setText(QString::number(calibration_factor * 100.0f) +
                               QString("%"));
}
...
void GLWidget::SetGenerateBranchingScaleChild(const int i)
{
    ...
    branching_scalechild_label->setText(
        QString::number(generate_branching_scalechild));
...
void GLWidget::SetGenerateRevolveSections(const int i)
{
    ...
    num_revolve_sections_label->setText(
        QString::number(generate_revolve_sections));
...
void GLWidget::SnapToMajorAxis()
{
    ...
    for (int i = 0; i < dirs.size(); ++i) {
        const float each_angle =
            GLutils::AngleBetweenRad(dirs[i], sections[selected].N());
    ...
    QVector3D rot_axis =
        QVector3D::crossProduct(dirs[closest_index], sections[selected].N())
            .normalized();

    QVector3D new_t =
        GLutils::RotateVector(sections[selected].T(), rot_axis, -closest_angle);
    QVector3D new_n =
        GLutils::RotateVector(sections[selected].N(), rot_axis, -closest_angle);
    QVector3D new_b =
        GLutils::RotateVector(sections[selected].B(), rot_axis, -closest_angle);
...
void GLWidget::LoadTemplateCurve()
{
    QString filename = QFileDialog::getOpenFileName(
        this, tr("Open Curve Template"), QString(""), tr("SVG (*.svg)"), 0,
    ...
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        qDebug() << "GLWidget::LoadTemplateOBJ(): File " << filename
                 << " can't be loaded";
        return;
    }
...
void GLWidget::LoadTemplateImage()
{
    ...
    QString filename = QFileDialog::getOpenFileName(
        this, tr("Open Image Template"), QString(""), tr("All Files (*)"), 0,
        QFileDialog::DontUseNativeDialog);
    if (filename.isNull()) {
        return;
    }
    ...
    if (GL_formatted_image.isNull()) {
        qDebug()
            << "GLWidget::LoadTemplateImage() - GL formatted image is NULL";
        return;
    }
...
{
    QString filename = QFileDialog::getOpenFileName(
        this, tr("Open Surface Template"), QString(""),
        tr("OBJ (*.obj);;All Files (*)"), 0, QFileDialog::DontUseNativeDialog);
    ...
    while (!ifs.atEnd()) {
        ...
        if (QString::compare(line.first(), "v") == 0) {
            ...
            for (int j = 0; j < line.size(); ++j) {
                ...
                if (v > 0) {  // indexes in OBJ should always be greater than 0,
                              // it's 0 if int conversion failed
                    face.push_back(v - 1);
                }
            }
            if (face.size() >= 3) {
            ...
            // for a face line, triangle face entries come from 1-2-3, 1-3-4,
            // 1-4-5, ...
            ...
            for (int j = 3; j < line.size(); ++j) {
                ...
                template_faces.push_back(ind_v1);  // note: 1-indexed in file,
                                                   // but 0-indexed in program
                template_faces.push_back(ind_v2);
                template_faces.push_back(ind_v3);

                if (each_v1.size() ==
                    3) {  // there is included normal information of format
                ...
                } else {  // we compute a per-face normal, but shading won't be
                          // smooth across faces, TODO: normal smoothing

                    const QVector3D dir1 =
                        (template_verts[ind_v2] - template_verts[ind_v1])
                            .normalized();
                    const QVector3D dir2 =
                        (template_verts[ind_v3] - template_verts[ind_v1])
                            .normalized();
    ...
    for (int i = 0; i < template_verts.size(); ++i) {
        ...
        template_verts[i].setX(template_verts[i].x() -
                               (bbox_max.x() + bbox_min.x()) *
                                   0.5f);  // move to X centroid
        template_verts[i].setY(template_verts[i].y() -
                               bbox_min.y());  // put above ground plane on Y
        template_verts[i].setZ(template_verts[i].z() -
                               (bbox_max.z() + bbox_min.z()) *
                                   0.5f);  // move to Z centroid
    ...
    qDebug() << "GLWidget::LoadTemplateOBJ(): File " << filename << "loaded.  "
             << template_verts.size() << "vertices,"
             << template_faces.size() / 3 << "faces.";
...
{
    QString filename =
        QFileDialog::getOpenFileName(this, tr("Open FlatFab"), QString(),
                                     tr("FlatFab (*.txt);;All Files (*)"), 0,
                                     QFileDialog::DontUseNativeDialog);
    ...
    QFile file(filename);
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        qDebug() << "GLWidget::LoadPlaneSketch(): File " << filename
                 << " can't be loaded";
        return false;
    }
...
bool GLWidget::SavePlaneSketch()
{
    // get filename
    QString filename =
        QFileDialog::getSaveFileName(this, tr("Save FlatFab"), QString(""),
                                     tr("FlatFab (*.txt);;All Files (*)"), 0,
                                     QFileDialog::DontUseNativeDialog);
    if (filename.isNull()) {
        return false;
    ...
    if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) {
        qDebug() << "GLWidget::SavePlaneSketch(): File " << filename
                 << " can't be saved";
        return false;
    }
...
void GLWidget::SaveSliceOBJ()
{
    QString filename = QFileDialog::getSaveFileName(
        this, tr("Save Slice OBJ"), QString(""),
        tr("OBJ (*.obj);;All Files (*)"), 0, QFileDialog::DontUseNativeDialog);
    ...
    if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) {
        qDebug() << "GLWidget::SaveSliceOBJ(): File " << filename
                 << " can't be exported";
        return;
    }
    ...
    for (int i = 0; i < sections.size(); ++i) {
        ...
        ofs << "g plane" << i << "\n";
        ofs << "vn " << sections[i].N().x() << " " << sections[i].N().y() << " "
            << sections[i].N().z() << "\n";
        ...
        for (int j = 0; j < tris.size(); j += 3) {
            ofs << "f " << face_count + 1 << "//" << i + 1 << " "
                << face_count + 2 << "//" << i + 1 << " " << face_count + 3
                << "//" << i + 1 << "\n";
            face_count += 3;
        }
    }
...
void GLWidget::SaveSlabOBJ()
{
    ...
    if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) {
        qDebug() << "GLWidget::SaveSlabOBJ(): File " << filename
                 << " can't be exported";
        return;
    }
    ...
    for (int i = 0; i < sections.size(); ++i) {
        ...
        for (int j = 0; j < vs.size(); j += 3) {
            ofs << "f " << vert_count + 1 << "//" << norm_count + 1 << " "
                << vert_count + 2 << "//" << norm_count + 1 << " "
                << vert_count + 3 << "//" << norm_count + 1 << "\n";
            vert_count += 3;
            ++norm_count;
        }
    }
...
{
    QString filename = QFileDialog::getSaveFileName(
        this, tr("Save Flattened Slab OBJ"), QString(""),
        tr("OBJ (*.obj);;All Files (*)"), 0, QFileDialog::DontUseNativeDialog);
    ...
    if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) {
        qDebug() << "GLWidget::SaveFlattenedSlabOBJ(): File " << filename
                 << " can't be exported";
        return;
    }
    ...
    // 2. write out each slab, however map each slab to a coordinate frame that
    // rests on the 2D ground plane
...
{
    QString filename = QFileDialog::getSaveFileName(
        this, tr("Save Surface OBJ"), QString(""),
        tr("OBJ (*.obj);;All Files (*)"), 0, QFileDialog::DontUseNativeDialog);
    ...
    if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) {
        qDebug() << "GLWidget::SaveSurfaceOBJ(): File " << filename
                 << " can't be exported";
        return;
    }
    ...
    ofs << "# Created with FlatFab\n";
    ofs << "# Mesh based on " << sections.size() << " planes and "
        << contour_graph.GetNumPatches() << "patches\n";
...
{
    QString filename = QFileDialog::getSaveFileName(
        this, tr("Save Physics Output"), QString(""),
        tr("TXT (*.txt);;All Files (*)"), 0, QFileDialog::DontUseNativeDialog);
    ...
    if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) {
        qDebug() << "GLWidget::SavePhysicsOutput(): File " << filename
                 << " can't be exported";
        return;
    }
    ...
    for (int i = 0; i < sections.size(); ++i) {
        ...
        const QVector3D mass_cent =
            sections[i].GetCentroid3D();  // centre of mass
        ofs << "PlaneCentreOfMass3D " << mass_cent.x() << " " << mass_cent.y()
            << " " << mass_cent.z() << "\n";
        ...
        for (int j = 0; j < contact_pts.size(); ++j) {
            ofs << contact_pts[j].x() << " " << contact_pts[j].y() << " "
                << contact_pts[j].z() << "\n";
        }
    ...
    for (int i = 0; i < sections.size(); ++i) {
        ...
        for (int j = i + 1; j < sections.size(); ++j) {
            ...
            if (my_slots.size() >= 2) {
                ...
                joint_axis.push_back((sections[i].GetPoint3D(my_slots[1]) -
                                      sections[i].GetPoint3D(my_slots[0]))
                                         .normalized());
                joint_p1_pt1.push_back(my_slots[1]);
                joint_p1_pt2.push_back(my_slots[0]);

                joint_p2_pt1.push_back(sections[j].GetPoint2D(
                    sections[i].GetPoint3D(my_slots[1])));
    ...
    for (int i = 0; i < joint_p1i.size(); ++i) {
        ofs << "JointPlaneIndexes " << joint_p1i[i] << " " << joint_p2i[i]
            << "\n";
        ofs << "JointCentre3D " << joint_centre[i].x() << " "
            << joint_centre[i].y() << " " << joint_centre[i].z() << "\n";
        ofs << "JointAxis3D " << joint_axis[i].x() << " " << joint_axis[i].y()
            << " " << joint_axis[i].z() << "\n";
        ofs << "JointPlane1SlitEndpoints2D " << joint_p1_pt1[i].x() << " "
            << joint_p1_pt1[i].y() << " " << joint_p1_pt2[i].x() << " "
            << joint_p1_pt2[i].y() << "\n";
        ;
        ofs << "JointPlane2SlitEndpoints2D " << joint_p2_pt1[i].x() << " "
            << joint_p2_pt1[i].y() << " " << joint_p2_pt2[i].x() << " "
            << joint_p2_pt2[i].y() << "\n";
    }
...
void GLWidget::SaveSVG()
{
    ...
    int svg_dpi = QInputDialog::getInt(
        this, QString("Save SVG File - DPI"),
        QString("Enter DPI (dots per inch) for SVG.\n(Illustrator and many "
                "other programs use 72dpi (default), Inkscape uses 90dpi.)"),
        72, 1, 1000, 1, &ok, Qt::Dialog);
    if (!ok) {
        return;
    }

    double width_height_ratio = QInputDialog::getDouble(
        this, QString("Save SVG File - Aspect Ratio"),
        QString("Enter cutting bed width to height ratio.\n(For example, a "
                "24\"width X 12\"height ratio would be 2.0.)"),
        1.5, 0.1, 10.0, 1, &ok, Qt::Dialog);
    if (!ok) {
        return;
    }
    ...
    reply = QMessageBox::question(
        this, QString("Save SVG File - Numeric Labels"),
        QString("Do you want numeric labels to appear in the file?\n(Numeric "
                "labels help simplify the assembly process.)"),
    ...
    QString filename = QFileDialog::getSaveFileName(
        this, tr("Save SVG File"), QString(""),
        tr("SVG (*.svg);;All Files (*)"), 0, QFileDialog::DontUseNativeDialog);
    if (filename.isNull()) {
        return;
    }
    ...
    if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) {
        qDebug() << "GLWidget::SaveSVG(): File " << filename
                 << " can't be exported";
        return;
    }
    ...
    QTextStream ofs(&file);
    PlanarSection::SaveToSVG(sections, ofs, metres_per_unit, svg_dpi,
                             width_height_ratio, use_numeric_labels, labels);
    file.close();
...
void GLWidget::SaveCalibration()
{
    ...
    int svg_dpi = QInputDialog::getInt(
        this, QString("Save Calibration SVG File - DPI"),
        QString("Enter DPI (dots per inch) for SVG.\n(Illustrator and many "
                "other programs use 72dpi (default), Inkscape uses 90dpi.)"),
        72, 1, 1000, 1, &ok, Qt::Dialog);
    if (!ok) {
    ...
    int calib_spacing = QInputDialog::getInt(
        this, QString("Save Calibration SVG File - Spacing value"),
        QString("Enter lower bound for calibration value."), 70, 1, 99, 1, &ok,
        Qt::Dialog);
    if (!ok) {
        return;
    }

    int calib_increment = QInputDialog::getInt(
        this, QString("Save Calibration SVG File - Increment value"),
        QString("Enter increment for calibration value."), 2, 1, 20, 1, &ok,
        Qt::Dialog);
    if (!ok) {
        return;
    }
    ...
    // add extension if missing
    const QString extension(".svg");
    if (QString::compare(filename.right(extension.length()), extension,
    ...
    if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) {
        qDebug() << "GLWidget::SaveCalibration(): File " << filename
                 << " can't be exported";
        return;
    }
...
void GLWidget::SaveDXF()
{
    bool ok;
    double width_height_ratio = QInputDialog::getDouble(
        this, QString("Save DXF File - Aspect Ratio"),
        QString("Enter cutting bed width to height ratio.\n(For example, a "
                "24\"width X 12\"height ratio would be 2.0.)"),
        1.5, 0.1, 10.0, 1, &ok, Qt::Dialog);
    if (!ok) {
    ...
    QMessageBox::StandardButton reply;
    reply = QMessageBox::question(
        this, QString("Save DXF File - Numeric Labels"),
        QString("Do you want numeric labels to appear in the file?\n(Numeric "
                "labels help simplify the assembly process.)"),
        QMessageBox::No | QMessageBox::Yes);
    use_numeric_labels = (reply == QMessageBox::Yes);
    ...
    QString filename = QFileDialog::getSaveFileName(
        this, tr("Save DXF File"), QString(""),
        tr("DXF (*.dxf);; All Files (*)"), 0, QFileDialog::DontUseNativeDialog);
    if (filename.isNull()) {
        return;
    }
    ...
    if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) {
        qDebug() << "GLWidget::SaveDXF(): File " << filename
                 << " can't be exported";
        return;
    }
    ...
    QTextStream ofs(&file);
    PlanarSection::SaveToDXF(sections, ofs, metres_per_unit, dpi,
                             width_height_ratio, use_numeric_labels, labels);
    file.close();
...
void GLWidget::UpdateMarkers()
{
    ...
    if (sections.size() >= 1 &&
        (state == STATE_SLOT || state == STATE_CAM_TRANSLATE ||
         state == STATE_DEADZONE || state == STATE_CURVE ||
         state == STATE_PEN_POINT || state == STATE_PEN_DRAG)) {
    ...
    if (!sections.empty() &&
        (state == STATE_RECURSIVE_SETUP_SLOT || state == STATE_RECURSIVE_SLOT ||
         recursive_slot_start != QVector3D(0, 0, 0))) {
...
void GLWidget::DrawMarkers()
{
    ...
    for (int i = 0; i < markers2.size(); ++i) {
        ...
        glColor3f(markers_col2[i].x(), markers_col2[i].y(),
                  markers_col2[i].z());
...
void GLWidget::DrawDimensionTool()
{
    ...
    */
    ...
    GLutils::DrawCylinder(dimensiontool_start, dimensiontool_end,
                          cam.CamWidth() / 150.0f);
}
...
void GLWidget::DrawTemplateImage()
{
    ...
    const float w = 1.0f;
    const float h =
        float(template_image.height()) / float(template_image.width());
    ...
    t = GLutils::RotateVector(t, n, template_rotation * 3.14159f / 180.0f) *
        template_scale;
    b = GLutils::RotateVector(b, n, template_rotation * 3.14159f / 180.0f) *
        template_scale;
...
void GLWidget::DrawTemplateSurface()
{
    ...
    for (int i = 0; i < template_faces.size(); i += 3) {
        for (int j = 0; j < 3; ++j) {
            const int ind_v = template_faces[i + j];
            const int ind_vn = template_facenorms[i + j];
            glNormal3f(template_norms[ind_vn].x(), template_norms[ind_vn].y(),
                       template_norms[ind_vn].z());
            glVertex3f(template_magnetic_verts[ind_v].x(),
                       template_magnetic_verts[ind_v].y(),
                       template_magnetic_verts[ind_v].z());
        }
    }
    glEnd();
...
void GLWidget::UpdateMagneticCutSurface(const QList<QVector3D> &plane_ns,
                                        const QList<QVector3D> &plane_ps)
{
    for (int i = 0; i < template_magnetic_verts.size(); ++i) {
        ...
        for (int j = 0; j < plane_ps.size(); ++j) {
            ...
            const float interp_val = (1.0f - (fabsf(dist) / magnet_strength)) *
                                     4.0f;  // this makes it smoother

            if (fabsf(dist) < magnet_strength) {
                too_far = false;

                // Rule: within magnet field, vertex should move onto side of
                // plane that the vert's normal points toward

                // Vector3 side = planarCut.params.norm *
                // planarCut.params.norm.Dot(verts[i] - planarCut.params.cent);
                const QVector3D side = pl_n * dist;
                const QVector3D sidenorm = side.normalized();

                // qDebug() << "interpval" << interp_val << "dist" << dist <<
                // "magnetval" << magnet_strength;
                const QVector3D new_pt =
                    (p - side) - sidenorm * QVector3D::dotProduct(sidenorm, n) *
                                     magnet_strength;

                if (interp_val > 0.0f &&
                    interp_val < 1.0f) {  // we linterp close to the boundary of
                                          // the field
        ...
        // 2. use Karan's "wires" technique to blend each deformed points
        // together into a single coherent deformation
        /*
...
void GLWidget::ComputeTemplateCut(const QVector3D &n, const QVector3D &p,
                                  QList<QVector3D> &cut_segments)
{
    ...
    for (int i = 0; i < template_faces.size(); i += 3) {
        ...
        // intersection between plane and triangle, but remove zero-length
        // segments
        if (b1 && b2 && int1 != int2) {
...
void GLWidget::DrawTemplateCut(const QList<QVector3D> &cut_segments)
{
    ...
    for (int i = 0; i < cut_segments.size(); ++i) {
        glVertex3f(cut_segments[i].x(), cut_segments[i].y(),
                   cut_segments[i].z());
    }
...
void GLWidget::AcceptPenCurve()
{
    ...
    if (!active_section.SliceTriangles().empty()) {
        ...
        // for the added curve, we now update our physical tests (TODO:
        // checkbox/boolean to disable this)
        UpdateAllTests();
    }
    ...
    if (ShowGenerate()) {
        switch (gen_state) {
            ...
            case GENSTATE_GRID:
                ...
                sections.removeAt(generate_selections
                                      [0]);  // Note: this must come after
                                             // AddToUndoList(OP_GENERATE_GRID);
                                             // or else the undo will not work
                break;
...
bool GLWidget::ShowGenerate()
{
    ...
    switch (gen_state) {
        case GENSTATE_SLICES: {
            ...
            if (template_verts.empty() || template_faces.empty()) {
                ...
                if (current_tool_state ==
                    TOOLSTATE_DEFAULT) {  // this is should only be shown during
                                          // AcceptGenerate()
                    QMessageBox::warning(
                        this, "Generate Slices Error",
                        "Can't Accept Generate Slices - Template is empty "
                        "(either no vertices or faces).");
                }
            ...
            // 2.  we sweep through each of X, Y, and Z adding planar sections
            // along spacing defined by the slices_spacing value float
            // generate_slices_spacing; bool generate_slices_x; bool
            // generate_slices_y; bool generate_slices_z;
            for (int i = 0; i < 3; ++i) {
                ...
                // 2b.  Compute interval range to iterate over (the bounding box
                // dimensions along sampling direction n)
                ...
                // 2c.  iterate by the spacing amount, creating each planar
                // section
                for (int j = j_min; j <= j_max; ++j) {
                    if (i == 1 && j <= 0) {  // skip slices that would be below
                                             // the ground plane
                        continue;
                    }
                    ...
                    // 2c.1.  Compute the segment list with the plane
                    // intersected with triangles of the surface
        ...
        case GENSTATE_LINEAR: {
            if (generate_selections.size() < 2) {
                qDebug() << "GLWidget::DoGenerateLinear() - Warning, 2 "
                            "sections need to be selected";
                ...
                if (current_tool_state ==
                    TOOLSTATE_DEFAULT)  // this is should only be shown during
                                        // AcceptGenerate()
                    QMessageBox::warning(this, "Generate Linear Error",
                                         "Can't Accept Generate Linear - 2 "
                                         "sections need to be selected.");
            ...
            if (ind1 == ind2) {
                qDebug() << "GLWidget::DoGenerateLinear() - Last two "
                            "selections the same, aborting";
                ...
                if (current_tool_state ==
                    TOOLSTATE_DEFAULT)  // this is should only be shown during
                                        // AcceptGenerate()
                    QMessageBox::warning(this, "Generate Linear Error",
                                         "Can't Accept Generate Linear - Last "
            ...
            if (branch.contains(
                    ind1)) {  // node shouldn't have parent in its branch, this
                              // means a cycle, just use the 1 node
                branch.clear();
            ...
            spine_section.GetContourIntersections(copy_section, intersects,
                                                  intersects_which);

            if (intersects.size() < 2) {
                qDebug() << "GLWidget::DoGenerateLinear() - Aborting, need at "
                            "least 2 contour intersections.";
                ...
                if (current_tool_state ==
                    TOOLSTATE_DEFAULT)  // this is should only be shown during
                                        // AcceptGenerate()
                    QMessageBox::warning(
                        this, "Generate Linear Error",
                        "Can't Accept Generate Linear - Section 1 and Section "
                        "2 must intersect.");
            ...
            QVector2D isec_dir_2d =
                spine_section.GetPoint2D(intersects.last()) -
                spine_section.GetPoint2D(intersects.first());

            // compute bounding region
            QVector2D copy_normal_2d =
                spine_section.GetPoint2D(copy_section.N()) -
                spine_section.GetPoint2D(QVector3D(0, 0, 0));
            float min_val, max_val;
            spine_section.GetBoundingInterval(copy_normal_2d, min_val, max_val);

            const float copy_val = QVector2D::dotProduct(
                copy_normal_2d, spine_section.GetPoint2D(copy_section.P()));

            // we need to perform a test to determine which side of the contour
            // to attach to consistently when we cast rays create a ray on the
            // plane of spine_section
            QList<QVector2D> contour_isecs;
            spine_section.ComputeContourLineIntersects(
                isec_p1_2d, isec_p2_2d - isec_p1_2d, contour_isecs);
            // which contour_isec is closer to intersects[1]? // TODO: make this
            // more robust, this choice is arbitrary, and depends on spline
            // before point being defined
            const float contour_first_len =
                (spine_section.GetPoint3D(contour_isecs.first()) -
                 intersects[1])
                    .length();
            const float contour_last_len =
                (spine_section.GetPoint3D(contour_isecs.last()) - intersects[1])
                    .length();
            const float contour_scale =
                (spine_section.GetPoint3D(contour_isecs.last()) -
                 spine_section.GetPoint3D(contour_isecs.first()))
                    .length();
            const bool use_first_contour_isec =
                (contour_first_len < contour_last_len);

            // compute series of 2D intersections on spine's plane, along
            // direction ld (projected to 2D)
            const int nBefore =
                -(int((copy_val - min_val) / generate_linear_spacing) + 1);
            const int nAfter =
                int((max_val - copy_val) / generate_linear_spacing) + 1;
            ...
            for (int i = nBefore; i <= nAfter; ++i) {
                ...
                const QVector3D new_plane_p1 =
                    spine_section.GetPoint3D(copy_normal_2d * d);
                const QVector3D new_plane_p2 =
                    spine_section.GetPoint3D(copy_normal_2d * d + isec_dir_2d);
                ...
                // cast the ray and get the intersections with spine_section's
                // contour
                QList<QVector2D> contour_isecs;
                spine_section.ComputeContourLineIntersects(
                    ray_p1, ray_p2 - ray_p1, contour_isecs);
                ...
                if (!contour_isecs.empty()) {
                    ...
                    for (int j = 0; j < contour_isecs.size(); ++j) {
                        contour_isecs_3d.push_back(
                            spine_section.GetPoint3D(contour_isecs[j]));
                    }
                    ...
                    */
                    ...
                    // line of intersection distance is closest the piece we are
                    // cloning
                    const float each_contour_scale =
                        (contour_isecs_3d.last() - contour_isecs_3d.first())
                            .length() /
                        contour_scale;
                    ...
                    if (use_first_contour_isec) {  // when scaling Y a
                                                   // translation is required so
                                                   // the section will still
                                                   // intersect at the slit
                        newbasis_p = contour_isecs_3d.first() +
                                     (copy_section.P() - intersects[1]) +
                                     (intersects[1] - intersects[2]) *
                                         (1.0f - each_contour_scale);
                    } else {
                        newbasis_p = contour_isecs_3d.last() +
                                     (copy_section.P() - intersects[2]) +
                                     (intersects[2] - intersects[1]) *
                                         (1.0f - each_contour_scale);
                    }
                    ...
                    for (int j = 0; j < branch.size(); ++j) {
                        ...
                        const float scale_x = (generate_linear_scalex)
                                                  ? each_contour_scale
                                                  : 1.0f;
                        const float scale_y = (generate_linear_scaley)
                                                  ? each_contour_scale
                                                  : 1.0f;

                        const QVector3D new_t = GLutils::GetVectorNewBasis(
                            oldbasis_t, oldbasis_n, oldbasis_b,
                            QVector3D(0, 0, 0), newbasis_t, newbasis_n,
                            newbasis_b, QVector3D(0, 0, 0), copy_sec.T(), 1.0f,
                            1.0f, 1.0f);
                        const QVector3D new_n = GLutils::GetVectorNewBasis(
                            oldbasis_t, oldbasis_n, oldbasis_b,
                            QVector3D(0, 0, 0), newbasis_t, newbasis_n,
                            newbasis_b, QVector3D(0, 0, 0), copy_sec.N(), 1.0f,
                            1.0f, 1.0f);
                        const QVector3D new_b = GLutils::GetVectorNewBasis(
                            oldbasis_t, oldbasis_n, oldbasis_b,
                            QVector3D(0, 0, 0), newbasis_t, newbasis_n,
                            newbasis_b, QVector3D(0, 0, 0), copy_sec.B(), 1.0f,
                            1.0f, 1.0f);
                        const QVector3D new_p = GLutils::GetVectorNewBasis(
                            oldbasis_t, oldbasis_n, oldbasis_b, oldbasis_p,
        ...
        case GENSTATE_BLEND: {
            if (generate_selections.size() < 3) {
                qDebug() << "GLWidget::DoGenerateBlend() - Warning, 3 sections "
                            "need to be selected";
                ...
                if (current_tool_state ==
                    TOOLSTATE_DEFAULT)  // this is should only be shown during
                                        // AcceptGenerate()
                    QMessageBox::warning(this, "Generate Blend Error",
                                         "Can't Accept Generate Blend - 3 "
                                         "sections need to be selected.");

                return false;
            }

            // 2.  need to ensure that sections 2 and 3 are connected to section
            // 1
            ...
            if (!section1.IsIntersectingSection(section2) ||
                !section1.IsIntersectingSection(section3)) {
                qDebug() << "GLWidget::DoGenerateBlend() - Warning, section1 "
                            "does not intersect both section2 and section3";
                ...
                if (current_tool_state ==
                    TOOLSTATE_DEFAULT)  // this is should only be shown during
                                        // AcceptGenerate()
                    QMessageBox::warning(
                        this, "Generate Blend Error",
                        "Can't Accept Generate Blend - Section 1 does not "
                        "intersect both Section 2 and Section 3.");
            ...
            */
            ...
            section1.GetSectionsAlongCurve(
                section2, curve, generate_blend_sections - 1, new_sections);

            // 4.  set up first and last sections so their normals point
            // tangential to the curve (roughly) and they are centered on the
            // curve
            //   4a.  translate P of the first and last so they are on the curve
            //   (and the interpolated ones are translated correctly)
            section2.MoveP(new_sections.first().P());
            section3.MoveP(new_sections.last().P());

            //  4b.  set the normal of the first and last so that they are
            //  facing along the curve
            if (QVector3D::dotProduct(section2.N(), new_sections.first().N()) <
                0.0f) {
                section2.FlipN();
                section2.UpdateCurveTrisSlab();
            }
            if (QVector3D::dotProduct(section3.N(), new_sections.last().N()) <
                0.0f) {
                section3.FlipN();
                section3.UpdateCurveTrisSlab();
            }
            if (QVector3D::dotProduct(section2.T(), new_sections.first().T()) <
                0.0f) {
            ...
            //   5a.  correspondence depends on same # of control points.
            //   subdivide section with more, splitting at longest segments
            ...
            //  mapping ctrl points from 2 to 3)
            int corresp_offset;
            ...
            float theta1 = GLutils::SignedAngleBetweenRad(
                section2.N(), new_sections.first().N(), section2.T());
            float theta2 = GLutils::SignedAngleBetweenRad(
                section3.N(), new_sections.last().N(), section2.T());
            ...
            for (int i = 1; i < generate_blend_sections - 1; ++i) {
                ...
                curve2.InterpolateCurvePointCorrespondence(
                ...
                QVector3D rotate_b = GLutils::RotateVector(
                    new_sections[i].B(), section2.T(), -theta_t);
        ...
        case GENSTATE_REVOLVE: {
            if (generate_selections.size() < 2) {
                qDebug() << "GLWidget::DoGenerateRevolve() - Warning, 2 "
                            "sections need to be selected";
                ...
                if (current_tool_state ==
                    TOOLSTATE_DEFAULT)  // this is should only be shown during
                                        // AcceptGenerate()
                    QMessageBox::warning(this, "Generate Revolve Error",
                                         "Can't Accept Generate Revolve - 2 "
                                         "sections need to be selected");
            ...
            if (!section1.IsIntersectingSection(section2)) {
                qDebug() << "GLWidget::DoGenerateRevolve() - Warning, section1 "
                            "does not intersect section2";
                ...
                if (current_tool_state ==
                    TOOLSTATE_DEFAULT)  // this is should only be shown during
                                        // AcceptGenerate()
                    QMessageBox::warning(
                        this, "Generate Revolve Error",
                        "Can't Accept Generate Revolve - Section 1 does not "
                        "intersect Section 2.");
            ...
            QList<PlanarSection> new_sections;
            section1.GetSectionsAlongCurve(
                section2, curve, generate_revolve_sections, new_sections);
            ...
            float theta = GLutils::SignedAngleBetweenRad(
                section2.N(), new_sections.first().N(), section2.T());
            ...
            for (int i = 1; i < generate_revolve_sections;
                ...
                const QVector3D newbasis_n = GLutils::RotateVector(
                    new_sections[i].N(), section2.T(), -theta);
                const QVector3D newbasis_b = GLutils::RotateVector(
                ...
                for (int j = 0; j < branch.size();
                    ...
                    const QVector3D new_t = GLutils::GetVectorNewBasis(
                        oldbasis_t, oldbasis_n, oldbasis_b, QVector3D(0, 0, 0),
        ...
        case GENSTATE_GRID: {
            if (generate_selections.size() < 1) {
                qDebug() << "GLWidget::DoGenerateGrid() - Warning, 1 section "
                            "must be selected";
                ...
                if (current_tool_state ==
                    TOOLSTATE_DEFAULT) {  // this is should only be shown during
                                          // AcceptGenerate()
                    QMessageBox::warning(this, "Generate Grid Error",
                                         "Can't Accept Generate Grid - 1 "
            ...
            // class, and only parameters passed
            ...
            for (int i = 0; i < split_pts.size(); ++i) {
                ...
                for (int j = 0; j < split_sections.size(); ++j) {
                    ...
                    split_sections[j].SplitAlongLine(
                        split_pts[i], split_dirs[i], each_split_sections);
            ...
            // add a bunch of staples regularly, then, only keep the ones
            ...
            for (int i = 0; i < staple_sections.size(); ++i) {
                ...
                for (int j = 0; j < split_sections.size(); ++j) {
                    if (staple_sections[i].IsIntersectingSection(
                            split_sections[j])) {
...
void GLWidget::StartGenerateSlices()
{
    ...
    selected = -1;
    generate_selections.clear();
    ShowGenerate();  // note: since no selections are required, perform
...
void GLWidget::DoGenerateBranching()
{
    ...
    // if there are n planar sections now, and root has k children, new tree
    // consists of kn + 1 sections
    QList<PlanarSection> new_sections;
    ...
    for (int i = 1; i < intersectors.size(); ++i) {
        ...
        for (int j = 0; j < sections_to_use->size(); ++j) {
            QVector3D new_t = GLutils::GetVectorNewBasis(
                sections[0].T(), sections[0].N(), sections[0].B(),
                QVector3D(0, 0, 0), sections[i].T(), sections[i].N(),
                sections[i].B(), QVector3D(0, 0, 0), (*sections_to_use)[j].T(),
                1.0f, 1.0f, 1.0f);
            QVector3D new_n = GLutils::GetVectorNewBasis(
                sections[0].T(), sections[0].N(), sections[0].B(),
                QVector3D(0, 0, 0), sections[i].T(), sections[i].N(),
                sections[i].B(), QVector3D(0, 0, 0), (*sections_to_use)[j].N(),
                1.0f, 1.0f, 1.0f);
            QVector3D new_b = GLutils::GetVectorNewBasis(
                sections[0].T(), sections[0].N(), sections[0].B(),
                QVector3D(0, 0, 0), sections[i].T(), sections[i].N(),
                sections[i].B(), QVector3D(0, 0, 0), (*sections_to_use)[j].B(),
                1.0f, 1.0f, 1.0f);
            QVector3D new_p = GLutils::GetVectorNewBasis(
                sections[0].T(), sections[0].N(), sections[0].B(),
                sections[0].P(), sections[i].T(), sections[i].N(),
                sections[i].B(), sections[i].P(), (*sections_to_use)[j].P(),
                generate_branching_scalechild, generate_branching_scalechild,
...
void GLWidget::StartDimensioningTool() { state = STATE_DIMENSIONING_FIRST; }
...
void GLWidget::DoPhysicsTest()
{
    ...
    for (int i = 0; i < sections.size(); ++i) {
        ...
        for (int j = 0; j < contact_pts.size(); ++j) {
            contact_pts_stlvector[j * 3 + 0] =
                contact_pts[j].x() * metres_per_unit;
            contact_pts_stlvector[j * 3 + 1] =
                contact_pts[j].y() * metres_per_unit;
            contact_pts_stlvector[j * 3 + 2] =
                contact_pts[j].z() * metres_per_unit;
        }
    ...
    for (int i = 0; i < sections.size(); ++i) {
        ...
        physics.AddPlate(p, t, n, b,
                         sections[i].SlabThickness() * metres_per_unit, rho,
                         pts);
    }
    ...
    for (int i = 0; i < sections.size(); ++i) {
        for (int j = i + 1; j < sections.size(); ++j) {
            ...
            if (my_slots.size() >= 2) {
                ...
                QVector2D p1_pt1 =
                    sections[j].GetPoint2D(sections[i].GetPoint3D(my_slots[1]));
                QVector2D p1_pt2 =
                    sections[j].GetPoint2D(sections[i].GetPoint3D(
                        my_slots[1] - my_slots[0] + my_slots[1]));
...
{
    cam.SetEye(default_lookat +
               QVector3D(0, 0, cam_lookat_distance));  // looking along z axis
    cam.SetLookAt(default_lookat);
...
void GLWidget::SetPhysicsNewWeightMass(const double d)
{
    ...
    new_weight_label->setText(QString::number(physics_new_weight_mass) +
                              QString(" kilograms"));
}
...
void GLWidget::DoGenerateMakeRadialHole()
{
    ...
    sections[selected].CreateRadialHoles(centre, rad, generate_radial_sectors,
                                         small_radii);
...
void GLWidget::DoGenerateSurfaceFacets()
{
    ...
    for (int i = 0; i < template_poly_faces.size(); ++i) {
        ...
        QVector3D new_n = QVector3D::crossProduct((v1 - v0).normalized(),
                                                  (v2 - v0).normalized())
                              .normalized();
        ...
        new_p /= face.size();
        // also offset it by half the material thickness (so adjacent sections
        // don't intersect one another) new_p += new_n * slab_thickness * 0.5f;
        new_section.SetP(new_p);

        // 1c.  Add control points to the section from the poly verts
        BezierCurve &curve = new_section.GetCurve(0);
        // curve.AddPoint(new_section.GetPoint2D(template_verts[face[0]]));
        // //add initial point
        ...
        for (int j = 0; j < face.size(); ++j) {
            ...
            // scale them in by the slab_thickness (allow a small gap so
            ...
            if (generate_surfacefacets_teeth) {
                ...
                for (int k = 0; k < teethpairs; ++k) {
                    ...
                    const float x1 =
                        float(k + 1) / float(teethpairs) * edge_len;
                ...
                curve.AddPoint(
                    v0 * 0.75f +
                    v1 * 0.25f);  // add 3 points for the Bezier segment
    ...
    // this block of code adds the circular coins at edges to join adjacent
    // faces
    if (!generate_surfacefacets_teeth) {

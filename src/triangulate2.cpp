...
bool Triangulate2::IsReflexAngle(const QVector2D & a, const QVector2D & b, const QVector2D & c)
{
    QVector2D ac = c - a;
    QVector2D ab = b - a;
...
bool Triangulate2::IsReflexIndex(const QList <QVector2D> & coords, const QList <int> polygon, const int pcurr)
{
    int pprev;
    int pnext;
...
float Triangulate2::IntersectSegmentX(const QVector2D & p0, const QVector2D & p1, const float y)
{
    if (p0.y() == p1.y()) {
        return p0.x();
    }
...
void Triangulate2::GetNeighbours(const QList <int> & polygon, const int pcurr, int & pprev, int & pnext)
{
...
void Triangulate2::GetSlice(const QList <QVector2D> & coords, const QList <int> & polygon, const QList <QVector2D> & hole, int slice[2])
{
...
bool Triangulate2::CheckEar(const QList <QVector2D> & coords, const QList <int> & polygon, const QList <bool> & reflex, const int reflexCount, const int pcurr)
{
    if (reflexCount == 0) {
        return true;
    }
...
bool Triangulate2::Process(const QList <QVector2D> & contour, QList <QVector2D> & result, QList <QVector2D> & poly)
{
...
bool Triangulate2::Process(const QList <QList <QVector2D> > & contours, QList <QVector2D> & result, QList <QVector2D> & poly)
{
    if (contours.empty()) {
        return false;
    }
...
bool Triangulate2::Process(QList <QVector2D> & coords, const QList <QList <QVector2D> > & holes, QList <QVector2D> & triangles, QList <QVector2D> & poly)
{
    ...
    //polygon = [0...coords.length] // the "..." means exclusive not inclusive range
    QList <int> polygon;
    ...
    for (int c=0; c<holes.size(); ++c) {
        ...
        const QList <QVector2D> & hole = holes[c]; //NOTE: this needs to generalize to more than 1 hole
    ...
    //qDebug() << "Ears:" << ears.size() << "Reflex:" << reflex.size() << reflexCount << "Convex:" << convex.size() << "Coords:" << coords.size();
    ...
    while (polygon.size() > 0) {
        ...
        if (ears.empty()) {
            //qDebug() << "Triangulate2::Process() - Warning, ears list empty, halting";
            break;
        }
        int pcurr = ears.last();
        ears.pop_back();
        ...
        for (int i=0; i<2; ++i) {
            ...
            if (neighbour >= 0 && neighbour < reflex.size() && reflex[neighbour] && !IsReflexIndex(coords, polygon, neighbour)) {
                reflex[neighbour] = false;
                --reflexCount;
            }

            if (neighbour < 0 || neighbour >= reflex.size() || !reflex[neighbour]) {
                bool isEar = CheckEar(coords, polygon, reflex, reflexCount, neighbour);

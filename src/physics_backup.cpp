                                   ...
                                   const int nH, const double max_stress)
{
    ...
    for (int ilf = 0; ilf < plt.alForce.size(); ilf++) {
        ...
        for (int ips = 0; ips < aPS.size(); ips++) {
            ...
            for (int iside = 0; iside < 2; iside++) {
                ...
                double dir[2] = {(ps.ep1.x - ps.ep0.x) / len,
                                 (ps.ep1.y - ps.ep0.y) / len};
    ...
    (ps.ep1.y-ps.ep0.y)*(ps.ep1.y-ps.ep0.y) ); double t = plt.thickness; double
    z0 = t*len*len/6.0; // section modulus double stress0 = fabs(ltlq.z / z0);
        double z1 = t*t*len/6.0; // section modulus
        double stress1 = fabs(ltlq.x*ny - ltlq.y*nx) / z1;
        ////
        double stress = (stress0>stress1) ? stress0 : stress1;
        //        std::cout << fabs(stress0) << " " << fabs(stress1) <<
    std::endl; if( stress > max_stress ){ plt.aPS.push_back(ps);
                                     ...
                                     const int nH, const double max_stress)
{
    ...
    for (int idir = 0; idir < ndir; idir++) {
        ...
        for (int ips = 0; ips < aPS.size(); ips++) {
            ...
            ps.weakness = stress / max_stress;
            if (stress > max_stress) {
                //      std::cout << (bm0-bm1).Length() << " " <<
                //      (bm0+bm1).Length() << std::endl;
                //        std::cout << len << " " << t << "   " << z0 << " " <<
                //        z1 << "   " << stress0 << " " << stress1 << std::endl;
                plt.aPS.push_back(ps);
            }
        }
    }
            ...
            if (is_draw_section_moment) {
                ...
                for (int ips = 0; ips < plt.aPS.size(); ips++) {
                    ...
                    GLutils::DrawArrowFixedLength(
                        ...
                        0.25f + 0.75f * qMin((ps.weakness - 1.0) / 4.0, 1.0));
                }
            }
        }
        if (is_draw_skeleton) {

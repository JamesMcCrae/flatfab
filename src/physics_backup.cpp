...
//    std::cout << ie0 << " " << r0 << std::endl;    std::vector<CPlateSection> aPS;
    ...
    for(int ips=0;ips<aPS.size();ips++){
      ...
      for(int iside=0;iside<2;iside++){
        ...
        double dir[2] = { (ps.ep1.x-ps.ep0.x)/len, (ps.ep1.y-ps.ep0.y)/len };
        ...
        double stress1 = fabs(ltrq.x*dir[0]+ltrq.y*dir[1]) / z1;
        ////
    ...
    /*
    for(int ips=0;ips<aPS.size();ips++){
      const CPlateSection& ps = aPS[ips];
      plt.aPS.push_back(ps);
    }
     */
  }
  ...
  for(int idir=0;idir<ndir;idir++){
    ...
    for(int ips=0;ips<aPS.size();ips++){
      ...
      const CVector3D ltlq = (bm1-bm0)*0.5;
      double len = sqrt( (ps.ep1.x-ps.ep0.x)*(ps.ep1.x-ps.ep0.x) + (ps.ep1.y-ps.ep0.y)*(ps.ep1.y-ps.ep0.y) );
      double t = plt.thickness;
      double z0 = t*len*len/6.0; // section modulus
      double stress0 = fabs(ltlq.z / z0);
      double z1 = t*t*len/6.0; // section modulus
      double stress1 = fabs(ltlq.x*ny - ltlq.y*nx) / z1;
      ////
      double stress = (stress0>stress1) ? stress0 : stress1;
      //        std::cout << fabs(stress0) << " " << fabs(stress1) << std::endl;
      if( stress > max_stress ){
        plt.aPS.push_back(ps);
        plt.alTorque.push_back(ltlq);
      }
    }
  }
   */
...
//      std::cout << bm1.x << " " << bm1.y << " " << bm1.z << std::endl;
      ...
      double stress0 = fabs(ltrq.z / z0);
      double z1 = t*t*len/6.0; // section modulus
      double stress1 = fabs(ltrq.x*ny - ltrq.y*nx) / z1;
      ////
      double stress = (stress0>stress1) ? stress0 : stress1;
      //       std::cout << fabs(stress0) << " " << fabs(stress1) << std::endl;
      ps.ltrq = ltrq;
      ps.weakness = stress/max_stress;
      if( stress > max_stress ){
//      std::cout << (bm0-bm1).Length() << " " << (bm0+bm1).Length() << std::endl;
//        std::cout << len << " " << t << "   " << z0 << " " << z1 << "   " << stress0 << " " << stress1 << std::endl;
        plt.aPS.push_back(ps);
      }
    }
  }  
...
void CPhysics::DrawEnhancedGL()
{
  const double small_rad = 0.05;
    ...
    if( aPlate.size() == aRigidBody.size() ){
      ...
      if( is_draw_section_moment ){
        ...
        for(int ips=0;ips<plt.aPS.size();ips++){
          ...
          //qDebug() << ps.weakness;
          GLutils::DrawArrowFixedLength(QVector3D(qm.x, qm.y, qm.z), QVector3D(qmtrq.x, qmtrq.y, qmtrq.z), 0.25f + 0.75f * qMin((ps.weakness-1.0)/4.0, 1.0));
        }
      }
    }
    if( is_draw_skeleton ){
    ...
    for(unsigned int icp=0;icp<rb.aCP.size();icp++){
      ...
      if( is_draw_force ){
        if (!rb.aCForce.empty()) {
            ...
            GLutils::DrawArrowFixedLength(a1, a2, 1.0f);
  ...
  for(int ij=0;ij<aJoint.size();ij++){
    ...
    if( is_draw_force ){
        ...
        GLutils::DrawArrowFixedLength(a1, a2,  0.5f);
        GLutils::DrawArrowFixedLength(a3, a4,  0.5f);
        ...
        GLutils::DrawCylinderFixedLength(a1, a5, small_rad*0.5f, 0.5f);
        GLutils::DrawCylinderFixedLength(a3, a6, small_rad*0.5f, 0.5f);
    }

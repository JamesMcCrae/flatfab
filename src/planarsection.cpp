...
BezierCurve & PlanarSection::GetCurve(const int i) { return bez_curve[i]; }

int PlanarSection::GetNumCurves() const { return bez_curve.size(); }
...
QList<BezierCurve> & PlanarSection::GetCurves() { return bez_curve; }
...
void PlanarSection::SetNewBasis(const QVector3D & new_t,
                                const QVector3D & new_n,
                                const QVector3D & new_b,
                                const QVector3D & new_p)
{
    for (int j = 0; j < bez_curve.size(); ++j) {
        const QList<QVector2D> & pts = bez_curve[j].Points();

        for (int i = 0; i < pts.size(); ++i) {
            QVector2D each_new_pt = GetPoint2D(GLutils::GetVectorNewBasis(
                t, n, b, p, new_t, new_n, new_b, new_p, GetPoint3D(pts[i]),
                1.0f, 1.0f, 1.0f));
...
void PlanarSection::SetP(const QVector3D & v) { p = v; }
...
void PlanarSection::CreateRadial(QVector2D centre, const float base_rad,
                                 const int num_sectors, const float radii[9])
{
    QList<QVector2D> pts;
    ...
    for (int i = 0; i < num_sectors; ++i) {
        for (int j = 0; j < 9; ++j) {
            const float t =
                (float(i) + float(j) / 9.0f) / float(num_sectors) * pi2;
...
void PlanarSection::CopySectorRadially(QVector2D centre, int num_sectors,
                                       int points_per_sector)
{
    if (!radial) {
    ...
    for (int i = 0; i < points_per_sector; i++) {
        lines.push_back(
            QLineF(centre.toPointF(), bez_curve[0].Point(i).toPointF()));
...
void PlanarSection::CreateRadialHoles(QVector2D centre, const float base_rad,
...
void PlanarSection::AddWeightAtMousePos(const QVector2D & mouse_pos,
                                        const float mass)
{
    QVector3D intersect;
...
int PlanarSection::GetNumWeights() { return weights.size(); }
...
float PlanarSection::GetWeightMass(const int i) { return weights[i].mass_kg; }
...
void PlanarSection::SetT(const QVector3D & v) { t = v; }

void PlanarSection::SetN(const QVector3D & v) { n = v; }

void PlanarSection::SetB(const QVector3D & v) { b = v; }

QVector3D PlanarSection::P() const { return p; }

QVector3D PlanarSection::T() const { return t; }

QVector3D PlanarSection::N() const { return n; }

QVector3D PlanarSection::B() const { return b; }
...
void PlanarSection::DrawCurveControlPointsHandleStyle(const float cam_width,
...
void PlanarSection::DrawShadow()
{
    ...
    for (int i = 0; i < slab_vert.size(); ++i) {
        ...
        glVertex3f(v.x() - qMax(float(v.y()), 0.0f), 0.0f,
                   v.z() + qMax(float(v.y()), 0.0f) / 2.0f);
    }
    glEnd();
}
...
void PlanarSection::DrawDeadzone(const QVector3D & slot_start,
                                 const QVector3D & slot_end,
                                 const float deadzone_radius)
{
...
void PlanarSection::DrawConvexHull(const QList<QVector3D> & hull,
                                   const bool draw_green)
{
    glEnable(GL_BLEND);
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
...
void PlanarSection::Update(const int c, const double error_tolerance)
{
    ...
    if (sketch_pts[c].size() > 5) {
        tan_end = (sketch_pts[c][sketch_pts[c].size() - 1] -
                   sketch_pts[c][sketch_pts[c].size() - 5])
                      .normalized();
    } else {
        tan_end = (sketch_pts[c][sketch_pts[c].size() - 1] -
                   sketch_pts[c][sketch_pts[c].size() - 2])
                      .normalized();
    }
...
int PlanarSection::LoadHeaderFromFile(QTextStream & ifs)
{
    ...
    line = ifs.readLine().split(" ");
    if (QString::compare(line.first(), "PlaneSketch") == 0 ||
        QString::compare(line.first(), "FlatFab") == 0) {
...
void PlanarSection::LoadFromFile(QTextStream & ifs)
{
    ...
    // determine what to do here... if we see "boundarycurves", holes are
    // supported if not, there are no holes, just a single boundary contour
    ...
    if (QString::compare(curves_line.first(), "BoundaryCurves") == 0) {
        ...
        for (int j = 0; j < nCurves; ++j) {
            ...
            for (int i = 0; i < nSketchPts; ++i) {
                line = ifs.readLine().split(" ");
                sketch_pts[j].push_back(
                    QVector2D(line[0].toFloat(), line[1].toFloat()));
            }
        ...
        for (int i = 0; i < nBezPts; ++i) {
            line = ifs.readLine().split(" ");
            bez_curve[0].AddPoint(
                QVector2D(line[0].toFloat(), line[1].toFloat()));
        }
    }
}

void PlanarSection::ComputePacking(const QList<PlanarSection> & sections,
                                   const double width_height_ratio,
                                   QVector<QVector2D> & pack_pts,
                                   QVector2D & bbox)
{
    qDebug() << "Packing" << sections.size()
             << "sections into rectangle of ratio" << width_height_ratio
             << "...";

    // First Fit Decreasing (FFD) strategy, operates by first sorting the items
    // to be inserted in decreasing order by their sizes, and then inserting
    // each item into the first bin in the list with sufficient remaining space.
    ...
    for (int i = 0; i < sections.size(); ++i) {
        ...
        for (int x = 0; x + bb[p1].x() < bbox.x(); ++x) {
            for (int y = 0; y + bb[p1].y() < bbox.y(); ++y) {
                ...
                bool collide = false;
                for (int j = 0; j < i;
                     ++j) {  // check all pieces already placed for collision
    ...
    qDebug() << "Completed packing inside rectangle of dimensions" << bbox.x()
             << bbox.y();
}

void PlanarSection::SaveToDXF(const QList<PlanarSection> & sections,
                              QTextStream & ofs, const double metres_per_unit,
                              const int, const double width_height_ratio,
                              const bool use_numeric_labels,
                              const QList<QString> & numeric_labels)
{
    ...
    for (int i = 0; i < sections.size(); ++i) {
        ...
        for (int c = 0; c < sections[i].bez_curve.size(); ++c) {
            const QList<QVector2D> & samples =
                sections[i].bez_curve[c].Samples();
        ...
        for (int j = 0; j < sections.size(); ++j) {
            ...
            for (int k = 0; k < slots_i_rect.size(); k += 5) {
                for (int l = 1; l < slots_i_rect[k].size(); ++l) {
                    ...
                    QVector3D p1 =
                        (slots_i_rect[k][i1] + pack_pts[i] - min_v) * scale;
                    QVector3D p2 =
                ...
                if (use_numeric_labels && !slots_i_rect[k].empty()) {
                    QVector2D dir =
                        (slots_i_rect[k][2] - slots_i_rect[k][1]).normalized();
                    QVector3D p1 =
                        ((slots_i_rect[k][2] + slots_i_rect[k][3]) * 0.5f +
                         pack_pts[i] - min_v) *
                            scale +
                        dir * 20.0f;
...
void PlanarSection::DXFWriteLine(QTextStream & ofs, const QVector3D & p1,
                                 const QVector3D & p2, const int layer_index,
                                 const int colour_index)
{
    ofs << "0\n";
    ofs << "LINE\n";
    ofs << "8\n";
...
void PlanarSection::DXFWriteText(QTextStream & ofs, const QVector3D & p1,
                                 const QString & text, const int layer_index,
                                 const int colour_index, const float height)
{
    ofs << "0\n";
    ofs << "TEXT\n";
    ofs << "8\n";
...
void PlanarSection::ComputeSlitNumericLabels(
    const QList<PlanarSection> & sections,
    const QList<QString> & numeric_labels,
    QVector<QVector<QString> > & graph_labels)
{
...
void PlanarSection::SaveToSVG(const QList<PlanarSection> & sections,
                              QTextStream & ofs, const double metres_per_unit,
                              const int dpi, const double width_height_ratio,
                              const bool use_numeric_labels,
                              const QList<QString> & numeric_labels)
{
    // so say we have this 2x2 box... we want each unit to be 1 inch
    ...
    ofs << "viewbox=\"0 0 " << bbox.x() * scale_inches << " "
        << bbox.y() * scale_inches << "\"\n";
    ofs << "id=\"svg2\">\n";
    ofs << "<defs\n";
    ofs << "id=\"defs4\" />\n";
    ...
    for (int i = 0; i < sections.size(); ++i) {
        ...
        for (int c = 0; c < sections[i].bez_curve.size(); ++c) {
            ...
            if (use_numeric_labels) {
                ofs << "style=\"fill:none;fill-rule:evenodd;stroke:#ff0000;"
        ...
        for (int j = 0; j < sections.size(); ++j) {
            ...
            for (int k = 0; k < slots_i_rect.size(); k += 5) {
                ...
                QVector2D p =
                    (slots_i_rect[k][0] + pack_pts[i] - min_v) * scale;
                ofs << "M " << p.x() << " " << p.y();
                ...
                if (use_numeric_labels && !slots_i_rect[k].empty()) {
                    QVector2D dir =
                        (slots_i_rect[k][2] - slots_i_rect[k][1]).normalized();
                    p = ((slots_i_rect[k][2] + slots_i_rect[k][3]) * 0.5f +
                         pack_pts[i] - min_v) *
                            scale +
                        dir * 20.0f;
                    ofs << "<text x=\"" << p.x() << "\" y=\"" << p.y()
                        << "\" text-anchor=\"middle\" ";
                    ofs << "style=\"font-size:16px;font-style:normal;font-"
                           "weight:normal;line-height:125%;letter-spacing:0px;"
                           "word-spacing:0px;fill:#0000ff;fill-opacity:1;"
...
void PlanarSection::GetSlots(const PlanarSection & other,
                             ...
                             QList<QList<QVector2D> > & my_slots_rect) const
{
    ...
    // 1.  get the slotline and ensure it has nonzero length (a ray is properly
    // defined)
    ...
    //     an interior region.  so we want to know the intersections of those
    //     interior regions/intervals, because at each one we will want to
    ...
    for (int i = 0; i + 1 < p0_isecs_3d.size(); i += 2) {
        ...
        for (int j = 0; j + 1 < p1_isecs_3d.size(); j += 2) {
            ...
            if (do_add_slot) {  // one of the conditions was satisfied, and the
                                // values were set
                ...
                slot_rect1.push_back(slot_start_2d + p0_slot_ortho *
                                                         other.SlabThickness() *
                                                         0.5f);
                slot_rect1.push_back(slot_start_2d - p0_slot_ortho *
                                                         other.SlabThickness() *
                                                         0.5f);
                slot_rect1.push_back(
                    slot_mid_2d - p0_slot_ortho * other.SlabThickness() * 0.5f);
                slot_rect1.push_back(
                    slot_mid_2d + p0_slot_ortho * other.SlabThickness() * 0.5f);
                slot_rect1.push_back(slot_start_2d + p0_slot_ortho *
                                                         other.SlabThickness() *
...
void PlanarSection::GetContourIntersections(const PlanarSection & other,
                                            QList<QVector3D> & isecs,
                                            QList<bool> & isecs_which) const
{
    isecs.clear();
    isecs_which.clear();
    ...
    for (int c = 0; c < bez_curve.size(); ++c) {
        ...
        for (int i = 0; i < samples.size(); ++i) {
            ...
            if (GLutils::LineSegmentPlaneIntersection(
                    other.p, other.n, GetPoint3D(samples[i]),
                    GetPoint3D(samples[j]), intersect)) {
    ...
    for (int c = 0; c < other.bez_curve.size(); ++c) {
        ...
        for (int i = 0; i < other_samples.size(); ++i) {
            ...
            if (GLutils::LineSegmentPlaneIntersection(
                    p, n, other.GetPoint3D(other_samples[i]),
                    other.GetPoint3D(other_samples[j]), intersect)) {
                isecs.push_back(intersect);
    ...
    for (const BezierCurve& c : bez_curve) {
        ...
        for (int i = 0; i < lines.size(); ++i) {
            ...
            for (int j = 0; j < samples.size(); ++j) {
                ...
                QVector2D intersect;
                if (GLutils::LineLineIntersection(lines[i], lines[l],
                                                  samples[j], samples[k],
                                                  intersect)) {
                    return true;
                }
...
void PlanarSection::GetBoundingInterval(const QVector2D & direction,
...
void PlanarSection::ComputeContourLineIntersects(
    const QVector2D & lp, const QVector2D & ld,
    QList<QVector2D> & intersects) const
{
    intersects.clear();
...
void PlanarSection::ComputeIntersectors(const QList<PlanarSection> & sections,
                                        const int which_section,
                                        QVector<bool> & intersectors)
{
    ...
    for (int j = 0; j < sections.size(); ++j) {
        ...
        intersectors[j] =
            sections[which_section].IsIntersectingSection(sections[j]);
    }
}

void PlanarSection::ComputeIntersectionGraph(
    const QList<PlanarSection> & sections, QVector<QVector<bool> > & graph)
{
    // init graph
    graph.clear();
...
void PlanarSection::TestConnectedness(QList<PlanarSection> & sections,
...
void PlanarSection::SetPartOfCycle(const bool b) { part_of_cycle = b; }

bool PlanarSection::PartOfCycle() const { return part_of_cycle; }

bool PlanarSection::CycleAssemblable(const QList<PlanarSection> & sections,
    ...
    for (int i = 0; i < cycle.size(); ++i) {
        for (int j = i + 1; j < cycle.size(); ++j) {
            const float dot_prod =
                QVector3D::dotProduct(int_dir[i], int_dir[j]);
...
void PlanarSection::SetConnected(const bool b) { connected = b; }

bool PlanarSection::Connected() const { return connected; }
...
void PlanarSection::SketchClear(const int c) { sketch_pts[c].clear(); }
...
void PlanarSection::SketchSetEditing(const bool b) { editing_sketch = b; }

int PlanarSection::SketchNumPoints() const { return sketch_pts[0].size(); }

bool PlanarSection::SketchEditing() { return editing_sketch; }
...
bool PlanarSection::CurveClockwise(const int c)
{
    ...
    total +=
        (pts.first().x() - pts.last().x()) * (pts.first().y() + pts.last().y());
...
void PlanarSection::SketchSymmetryTest()
{
    ...
    for (int i = 0; i < sketch_pts[0].size(); ++i) {
        // wrt local coordinate system, the region sidedness is based on the
        // x-coordinate of each point (a line where x=0 divides the two regions
        // in local 2D parameter space) qDebug() << i << sketch_pts[i].x();
    ...
    if ((dist_r1 > 0.0f && dist_r2 > 0.0f) ||
        (fabsf(dist_r2) / fabsf(dist_r1) >
         magic_ratio)) {  // mirror to region1 (negative half)
        sign = 1.0f;
    } else if ((dist_r1 < 0.0f && dist_r2 < 0.0f) ||
               (fabsf(dist_r1) / fabsf(dist_r2) >
                magic_ratio)) {  // mirror to region2 (positive half)
        sign = -1.0f;
    } else {
        return;
    }
    ...
    for (int i = nPts - 1; i >= 0; --i) {
        sketch_pts[0].push_back(
            QVector2D(-sketch_pts[0][i].x(), sketch_pts[0][i].y()));
    }
}
...
void PlanarSection::CreateLocalSymmetry()
{
    ...
    while (sign * bez_curve[0].Point(index).x() > 0.0f &&
           index < bez_curve[0].GetNumControlPoints() - 2) {
        new_points.push_back(bez_curve[0].Point(index - 1));
        new_points.push_back(bez_curve[0].Point(index));
        new_points.push_back(bez_curve[0].Point(index + 1));

        if (bez_curve[0].Point(index).x() <
            bez_curve[0].Point(smallest_x).x()) {
            smallest_x = index;
        }
    ...
    new_points.push_back(QVector2D(
        -bez_curve[0].Point(bez_curve[0].GetNumControlPoints() - 2).x(),
        bez_curve[0].Point(bez_curve[0].GetNumControlPoints() - 2).y()));
...
void PlanarSection::DrawSketch()
{
    for (int c = 0; c < sketch_pts.size(); ++c) {
        ...
        for (int i = 0; i < sketch_pts[c].size(); ++i) {
            QVector3D v =
                p + (t * sketch_pts[c][i].x()) + (b * sketch_pts[c][i].y());
            glVertex3f(v.x(), v.y(), v.z());
        }
        glEnd();
    }
...
bool PlanarSection::AddMouseRayIntersect(const int c, const QVector2D & v)
{
    ...
    if (result) {
        ...
        if (sketch_pts[c].empty() ||
            (new_pt - sketch_pts[c].last()).length() > min_dist_threshold) {
...
bool PlanarSection::AddCtrlPointPenPress(const int c, const QVector2D & v)
{
    ...
    if (result) {
        ...
        if (bez_curve[c].Points().size() > 3) bez_curve[c].SetClosed(true);
...
QVector2D PlanarSection::Centroid() const { return centroid; }

float PlanarSection::SignedArea() const { return area; }

QVector3D PlanarSection::GetCentroid3D() const { return GetPoint3D(centroid); }
...
const QList<QVector2D> & PlanarSection::SliceTriangles() const { return tris; }
...
float PlanarSection::SlabThickness() const { return slab_thickness; }

void PlanarSection::SetSlabThickness(const float f) { slab_thickness = f; }

bool PlanarSection::RayIntersect(const QVector3D & p0, const QVector3D & dir,
                                 QVector3D & intersect)
{
...
bool PlanarSection::MouseOutsideDeadzone(const QVector2D & v,
                                         const QVector3D & slot_start,
                                         const QVector3D & slot_end,
                                         const float deadzone_radius)
{
    QVector3D intersect;
    ...
    return GLutils::PointLineSegmentDistance(intersect, slot_start, slot_end) >
           deadzone_radius;
}

void PlanarSection::SelectMouseRayIntersect(const QVector2D & v,
                                            const float cam_width)
{
    QVector3D intersect;
    MouseRayIntersect(v, intersect);
    ...
    for (int c = 0; c < bez_curve.size(); ++c) {
        if (bez_curve[c].SelectedPoint() >= 0) {
            if (c > 0 && bez_curve[c].GetNumControlPoints() <= 7) {
                ...
                if (radial && bez_curve[c].SelectedPoint() <
                                  num_radial_points_per_sector) {
    ...
    if (bez_curve[bez_curve_index].GetNumControlPoints() <= 7) {
        if (bez_curve_index > 0) {
            ...
            if (bez_curve[bez_curve_index].GetNumControlPoints() <= 4)
                ...
                // Set closed allows DeletePoint to work for smaller than 7
                // points
...
bool PlanarSection::IsWeightSelected() { return selected_weight >= 0; }
...
void PlanarSection::MoveCtrlPointMouseRayIntersect(const QVector2D & mouse_pos,
                                                   const bool keep_g1,
...
void PlanarSection::MirrorPlanarSectionX(PlanarSection & new_section) const
{
    ...
    // since we mirror TNB frame, we need to swap T and B to keep handedness of
    // frame the same
...
{    
    ...
    for (int i = 0; i < slab_norm.size(); ++i) {
        for (int j = 0; j < 3; ++j) {
            ...
            glVertex3f(slab_vert[i * 3 + j].x(), slab_vert[i * 3 + j].y(),
                       slab_vert[i * 3 + j].z());
        }
    }
    glEnd();
    ...
    for (int c = 0; c < bez_curve.size(); ++c) {
        ...
        for (int j = 0; j < 2; ++j) {
            for (int i = 0; i < samples.size(); ++i) {
                ...
                QVector3D v2 =
                    p + (t * samples[i2].x()) + (b * samples[i2].y());
...
void PlanarSection::ComputeCentroidAndArea()
{
    ...
    for (int c = 0; c < bez_curve.size(); ++c) {
        ...
        for (int i = 0; i < samples.size(); ++i) {
            const int j = (i + 1) % samples.size();
            const float cross_prod = samples[i].x() * samples[j].y() -
                                     samples[j].x() * samples[i].y();

            each_area += cross_prod;

            each_centroid.setX(each_centroid.x() +
                               (samples[i].x() + samples[j].x()) * cross_prod);
            each_centroid.setY(each_centroid.y() +
                               (samples[i].y() + samples[j].y()) * cross_prod);
        }
...
void PlanarSection::ComputeSlab()
{
    ...
    for (int c = 0; c < bez_curve.size(); ++c) {
        ...
        for (int i = 0; i < samples.size(); ++i) {
            ...
            QVector3D each_n =
                QVector3D::crossProduct(cross2, cross1).normalized();
...
void PlanarSection::GetXZPlanePoints(QList<QVector3D> & plane_pts) const
{
    ...
    const QVector3D intersect_line =
        QVector3D::crossProduct(n, QVector3D(0, 1, 0));
    ...
    for (int j = 0; j < tris.size(); j += 3) {
        for (int k = 0; k < 3; ++k) {  // for each line of triangle
            ...
            if (GLutils::LineSegmentPlaneIntersection(
                    QVector3D(0, 0, 0), QVector3D(0, 1, 0), v1, v2, i1)) {
                i1.setY(0.0f);  // just to be sure

                const float each_val =
                    QVector3D::dotProduct(intersect_line, i1);
    ...
    // we are only interested in the two extremal points on this line (line
    // formed by intersection of each plane with ground plane)
...
void PlanarSection::GetXZPlanePoints(const QList<PlanarSection> & sections,
...
void PlanarSection::GetXZConvexHull(const QList<PlanarSection> & sections,
                                    QList<QVector3D> & convex_hull)
{
    QList<QVector3D> plane_pts;
    QList<int> hull;
...
bool PlanarSection::GetXZPointInsideConvexHull(const QList<QVector3D> & hull,
                                               const QVector3D & p)
{
...
void PlanarSection::SnapSketchToTemplateCut(
    const QList<QVector3D> & cut_segments, const float snap_dist_3d)
{
    ...
    for (int j = 0; j < cut_segments.size(); j += 2) {
        ...
        float each_dist =
            GLutils::PointLineSegmentDistance(pt3d, cut_segments[j],
                                              cut_segments[j + 1], each_pt) +
            GLutils::PointLineSegmentDistance(
                last_pt3d, cut_segments[j], cut_segments[j + 1], last_each_pt);
...
void PlanarSection::SaveConnectionsToOBJ(const QList<PlanarSection> & sections,
                                         QTextStream & ofs)
{
    ...
    for (int i = 0; i < sections.size(); ++i) {
        ofs << "# " << sections[i].N().x() << " " << sections[i].N().y() << " "
            << sections[i].N().z() << " " << sections[i].P().x() << " "
            << sections[i].P().y() << " " << sections[i].P().z() << "\n";
    }

    ofs << "# Connections " << conn1.size() << "\n";
    for (int i = 0; i < conn1.size(); ++i) {
        ofs << "# " << conn1[i] << " " << conn2[i] << " " << conn_start[i].x()
            << " " << conn_start[i].y() << " " << conn_start[i].z() << " "
            << conn_end[i].x() << " " << conn_end[i].y() << " "
            << conn_end[i].z() << "\n";
    }
}

void PlanarSection::SplitAlongLine(const QVector2D & split_p,
                                   const QVector2D & split_d,
                                   QList<PlanarSection> & split_sections)
{
    ...
    // 1. march clockwise along curve (starting anywhere). during marching test
    // to see if split line is crossed (if never crossed, no split)
    // 2. if the splitting line is crossed, march along splitting line (in
    // clockwise direction) until another part of the curve is encountered.
    // eventually we get back to 1st point we also want to continue along that
    // point of intersection along the curve, as this defines another piece on
    // the other side
...
void PlanarSection::GetCurveBetweenSections(const PlanarSection & section1,
                                            const PlanarSection & section2,
                                            BezierCurve & curve)
{
    // 1.  Find the segments and t values on the 2 curves that these sections
    // intersect.
    ...
    bez_curve[0].GetLineSegmentIntersections(slots1.first(), slots1.last(),
                                             sec1_curve_ints);
    bez_curve[0].GetLineSegmentIntersections(slots2.first(), slots2.last(),
                                             sec2_curve_ints);

    if (sec1_curve_ints.empty() || sec2_curve_ints.empty()) {
        qDebug() << "PlanarSection::GetCurveBetweenSections() - Warning, "
                    "GetLineSegmentIntersections returning empty";
        return;
    }
    ...
    bez_curve[0].GetSubCurve(sec1_curve_ints.first(), sec2_curve_ints.first(),
                             sub_curve1);  // march forward from 1->2
    bez_curve[0].GetSubCurve(sec2_curve_ints.first(), sec1_curve_ints.first(),
                             sub_curve2);
    sub_curve2.Reverse();  // want to preserve order of going from 1 -> 2, so
                           // reversing this is like marching backward from 1->2

    curve =
        (sub_curve1.Length() < sub_curve2.Length()) ? sub_curve1 : sub_curve2;
}

void PlanarSection::GetCurveAroundSection(const PlanarSection & section,
                                          BezierCurve & curve)
{
    // 1.  Find the segments and t values on the 2 curves that these sections
    // intersect.
    ...
    if (!GetIntersectionSlotLine(section, slot_start, slot_end)) {
        qDebug() << "PlanarSection::GetCurveAroundSection() - Warning, plane "
                    "intersection with section1 and section2 not found";
        return;
    }
    ...
    if (isecs.size() < 2) {
        qDebug() << "PlanarSection::GetCurveAroundSection() - Warning, "
    ...
    bez_curve[0].GetLineSegmentIntersections(
        GetPoint2D(isecs.first()), GetPoint2D(isecs.last()), sec_curve_ints);

    if (sec_curve_ints.empty()) {
        qDebug() << "PlanarSection::GetCurveAroundSection() - Warning, "
                    "GetLineIntersections returning empty";
        return;
    }
                                          ...
                                          QList<PlanarSection> & sections)
{
    ...
    for (int i = 0; i < num_frames + 1; ++i) {
        ...
        const QVector3D each_b =
            QVector3D::crossProduct(each_n, each_t).normalized();
...
void PlanarSection::DrawWeights()
{
    for (int i = 0; i < weights.size(); ++i) {
        ...
        GLutils::DrawRing(p2 - QVector3D(handle_thick, 0, 0),
                          p2 + QVector3D(handle_thick, 0, 0), handle_inner,
                          handle_outer);
    }
}
...
void PlanarSection::SetRadial(const bool b) { radial = b; }

bool PlanarSection::IsRadial() { return radial; }
...
int PlanarSection::GetNumRadialSectors() { return num_radial_sectors; }
...
{
    CopySectorRadially(radial_centre, num_radial_sectors,
                       num_radial_points_per_sector);
}

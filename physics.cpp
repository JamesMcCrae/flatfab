...
CPhysics::CPhysics()
{
  ...
  damping_ratio = 0.01;
  ////
  n = CVector3D(0,1,0); // normal direction of floor (should be an unit vector)
  ...
  is_slit = true;
  aDir2D.clear();
  aDir2D.push_back(1);             aDir2D.push_back(0);
  aDir2D.push_back(0);             aDir2D.push_back(1);
  aDir2D.push_back(0.70710678118); aDir2D.push_back(+0.70710678118);
  aDir2D.push_back(0.70710678118); aDir2D.push_back(-0.70710678118);
  nHeight = 20;
  //max_stress = 50000;
  max_stress = 60000000; //this is the value for acrylic, 60MPa (60 * 1,000,000)
}

//main class interface methods to set up problem

void CPhysics::AddPlate(const double p[3], const double t[3], const double n[3], const double b[3], const double thickness, const double rho, const std::vector<double> & pts)
{
                  ...
                  const std::vector<double>& contact_points)
{
  ...
  for(int icp=0;icp<ncp;icp++) {
    CVector3D pc(contact_points[icp*3+0],contact_points[icp*3+1],contact_points[icp*3+2]);
    rb.aCP.push_back(pc);
  }
  aRigidBody.push_back(rb);
}

void CPhysics::AddJoint(const double position[3], const double jp0[4], const double jp1[4], const int body_index1, const int body_index2)
{
...
void CPhysics::AddWeightToRigidBody(const double mass_pos[3], const double mass_magnitude[3])
{
    ...
    newWeight.second = CVector3D(mass_magnitude[0], mass_magnitude[1], mass_magnitude[2]);
...
void CPhysics::ReadFile(const char* fname) {
  ...
  for(int ij=0;ij<nj;ij++){
    ...
    fscanf(fp,"%s %lf %lf %lf %lf",buff,&d1,&d2,&d3,&d4); // JointPlane1SlitEndpoints2D
    j.jp0[0] = d1;
    j.jp0[1] = d2;
    j.jp0[2] = d3;
    j.jp0[3] = d4;
  ...
  for(int irb=0;irb<aRigidBody.size();irb++){
    {
      ...
      if( is_slit ){
        GetPlateLocalForce(aPlate[irb].alForce,
                           irb, aRigidBody, aJoint,
                           plt.p, plt.t, plt.b, plt.n,
                           plt.aXY_slit);
      }
      else{
        GetPlateLocalForce(aPlate[irb].alForce,
    ...
    if( is_slit ){
      AddWeakSection_Inside(aPlate[irb],
                            aPlate[irb].aXY_slit,
                            aRigidBody[irb],gravity,
                            aDir2D,nHeight,max_stress);
      // Sampling Around Slit
      AddWeakWection_Slit(aPlate[irb],
                          aPlate[irb].aXY_slit,
                          aRigidBody[irb],gravity,
                          aDir2D,nHeight,max_stress);
    }
...
void CPhysics::EdEddE_Exforce
(double& energy,
 CVector3D& dEdu, CVector3D& dEdw,
 ...
 )
{
  ...
  ddEddw  = cont_stiff*CMatrix3::OuterProduct(Rv^n,Rv^n) + ((cq*n)*cont_stiff)*CMatrix3::Spin(n)*CMatrix3::Spin(Rv);
...
//    ddEddw  = -cont_stiff*CMatrix3::OuterProduct(Rv,Rv) + cont_stiff*CMatrix3::Spin(Rv)*CMatrix3::Spin(cq-cp);
  ddEddw  = -cont_stiff*CMatrix3::Spin(Rv)*CMatrix3::Spin(Rv) + cont_stiff*CMatrix3::Spin(cq-cp)*CMatrix3::Spin(Rv);
  ddEdudw = cont_stiff*CMatrix3::Spin(Rv);
}


void CPhysics::EdEddE_Joint
(double& energy,
 CVector3D& dEdu0, CVector3D& dEdw0, CVector3D& dEdu1, CVector3D& dEdw1,
 ////
 ...
 )
{
  ...
  CVector3D qj0 = Rv0 + cg0 + u0; // after deformation joint pos relative to rigid body 0
  
  CVector3D Rv1 = R1 * (pj-cg1);
  CVector3D qj1 = Rv1 + cg1 + u1; // after deformation joint pos relative to rigid body 1
  ...
  ddEdw0dw0 = -trans_stiff*CMatrix3::Spin(Rv0)*CMatrix3::Spin(Rv0) + trans_stiff*CMatrix3::Spin(qj0-qj1)*CMatrix3::Spin(Rv0);
  ...
  ddEdw1dw1 = -trans_stiff*CMatrix3::Spin(Rv1)*CMatrix3::Spin(Rv1) + trans_stiff*CMatrix3::Spin(qj1-qj0)*CMatrix3::Spin(Rv1);
  ...
  for(unsigned int i=0;i<3;i++){
    ...
    ddEdw0dw0 += 0.5*rot_stiff*( CMatrix3::Spin(r1^av)*CMatrix3::Spin(r0) + CMatrix3::Spin(r0)*CMatrix3::Spin(r1)*davdw0 );
    ddEdw1dw1 -= 0.5*rot_stiff*( CMatrix3::Spin(r0^av)*CMatrix3::Spin(r1) + CMatrix3::Spin(r1)*CMatrix3::Spin(r0)*davdw1 );
    ddEdw0dw1 -= 0.5*rot_stiff*(  CMatrix3::Spin(r1)*CMatrix3::Spin(av)*CMatrix3::Spin(r0)
...
CMatrix3 CPhysics::rand_rot()
{
  ...
  v.y = s*rand()/(RAND_MAX+1.0);
  v.z = s*rand()/(RAND_MAX+1.0);
  CMatrix3 R;  R.SetRotMatrix_Cartesian(v);
  return R;
}
...
void CPhysics::CheckDiff_Contact()
{
  ...
  for(unsigned int idim=0;idim<3;idim++){
    ...
    CVector3D w(0,0,0);
    w[idim] = epsilon;
    CMatrix3 dR; dR.SetRotMatrix_Cartesian(w);
    CMatrix3 R_ = dR*R;
    EdEddE_Contact(energy_,  dEdu_,dEdw_,   ddEddu_,ddEddw_,ddEdudw_,
                     cg,cp,u,R_,
                     cont_stiff,n);
...
void CPhysics::AddMatrix(Eigen::MatrixXd& M,
 ...
 double nx, double ny)
{
  ...
  for(int ih=0;ih<nH;ih++){
    ...
    for(int ips_h=0;ips_h<nps_h;ips_h++){
      ...
      itr++;
      ps.ih = ih;
      ps.length = sqrt( (ps.ep0.x-ps.ep1.x)*(ps.ep0.x-ps.ep1.x) + (ps.ep0.y-ps.ep1.y)*(ps.ep0.y-ps.ep1.y) );
...
double CPhysics::GetMinDist_LineSegPoint2D
(double& t,
...
//    std::cout << "n: " << (pos_cp-vec_p)*vec_n << std::endl;
  ...
  /////////////////////////////////////////////////////////////////////////////////////////////////////////
  // if the force is on boundary, register its edge index and ratio on the edge
  //std::cout << "############### " << std::endl;
  ...
  for(int ilf=0;ilf<alForce.size();ilf++){
    ...
    for(int ie=0;ie<aXY.size()/2;ie++){
      int ip0 = ie;
      int ip1 = ie+1; if( ip1 == aXY.size()/2 ){ ip1 = 0; };
...
//      std::cout << ilf << "   boundary force  " << dist_min << " " << rep_len << " " << r_min << "  " << clf.is_trq << std::endl;
      clf.is_boundary = true;
      clf.iedge0 = ie_min;
      clf.ratio_edge0 = r_min;
    }
    else{
//      std::cout << ilf << "   non boundary force  " << dist_min << " " << rep_len << " " << r_min << "  " << clf.is_trq << std::endl;
    }
  }
}
...
double CPhysics::TriArea2D(const double v1[2], const double v2[2], const double v3[2])
{
...
bool CPhysics::IsHitRay(double& r,
                        const double org[2], const double end[2],
                        const double p0[ 2], const double p1[ 2])
{
  double a0 = TriArea2D(org,end,p0);
  double a1 = TriArea2D(end,org,p1);
  bool iflg = true;
  if( a0*a1 < 0 ){ iflg = false; }
  r = a0/(a0+a1);
  double pm[2] = {
    (1-r)*p0[0] + r*p1[0],
    (1-r)*p0[1] + r*p1[1] };
  double dot = (pm[0]-org[0])*(end[0]-org[0]) + (pm[1]-org[1])*(end[1]-org[1]);
  if( dot > 0 && iflg ){ return true; }
  return false;
}

bool CPhysics::IsForceInsideHalfLoop2D
(const std::vector<double>& aXY,
 ...
 const CPlateSection& ps)
{
  ...
  for(;;){
    unsigned int ip0 = ie;
    unsigned int ip1 = ip0+1; if( ip1 == aXY.size()/2 ){ ip1 = 0; }
    double p0[2] = { aXY[ip0*2+0], aXY[ip0*2+1] };
    double p1[2] = { aXY[ip1*2+0], aXY[ip1*2+1] };
    if(      ie == ie0 ){
      p0[0] = (1-re0)*p0[0] + re0*p1[0];
      p0[1] = (1-re0)*p0[1] + re0*p1[1];
      if( lf.is_boundary && lf.iedge0 == ie && lf.ratio_edge0 > re0 ){ return true; }
    }
    else if( ie == ie1 ){
    ...
    if( !lf.is_boundary ){
      double r;
      if( IsHitRay(r,org,end, p0,p1) ){ icnt++; }
    }
    if( ie == ie1 ) break;
    ie = ie+1;
    if( ie == aXY.size()/2 ){ ie = 0; }
  }
...
CVector3D CPhysics::MomentumTri
(double& a,
 CVector3D lg,
 double p0[2],
 double p1[2],
 double org[2])
{
  double cg[2] = { (p0[0]+p1[0]+org[0])/3.0, (p0[1]+p1[1]+org[1])/3.0 };
...
CVector3D CPhysics::MomentumHalfLoop2D
(double lx, double ly,
 const std::vector<double>& aXY,
 const CVector3D& lg,
 const CPlateSection& ps)
...
CVector3D CPhysics::MomentumLoop2D
(double lx, double ly,
 const std::vector<double>& aXY,
 const CVector3D& lg )
{
...
bool CPhysics::FirstHitRay
(int& ie0, double& r0,
 double org[2], double dir[2],
 const std::vector<double>& aXY)
{
  ...
  for(int ie=0;ie<aXY.size()/2;ie++){
    ...
    double p0[2] = { aXY[ip0*2+0], aXY[ip0*2+1] };
    double p1[2] = { aXY[ip1*2+0], aXY[ip1*2+1] };
    if( (p0[0]-org[0])*(p0[0]-org[0])+(p0[1]-org[1])*(p0[1]-org[1]) < sqdir*1.0e-10 ) continue;
    if( (p1[0]-org[0])*(p1[0]-org[0])+(p1[1]-org[1])*(p1[1]-org[1]) < sqdir*1.0e-10 ) continue;
    double r0;
    ...
    double pm[2] = {
      (1-r0)*p0[0] + r0*p1[0],
      (1-r0)*p0[1] + r0*p1[1] };
    double sqdist = (pm[0]-org[0])*(pm[0]-org[0]) + (pm[1]-org[1])*(pm[1]-org[1]);
...
void CPhysics::GetPlateSection_Slit(std::vector<CPlateSection>& aPS,
                                    int ie0,
                                    const std::vector<double>& aXY,
                                    bool is_loop_cc)
{
  int ip0 = ie0;
  int ip1 = ip0+1; if( ip1 == aXY.size()/2 ){ ip1 = 0; }
  double p0[2] = { aXY[ip0*2+0], aXY[ip0*2+1]};
  double p1[2] = { aXY[ip1*2+0], aXY[ip1*2+1]};
  ...
  eu[0] /= len;  eu[1] /= len;
  double ev[2];
  if( is_loop_cc ){ ev[0] = -eu[1];  ev[1] = +eu[0]; }
  else{             ev[0] = +eu[1];  ev[1] = -eu[0]; }
  ...
  for(int ismpl=0;ismpl<nSmpl;ismpl++){
    ...
    }
    {
      ...
      double dir[2] = {
        ev[0]*cos(tht) + eu[0]*sin(tht),
        ev[1]*cos(tht) + eu[1]*sin(tht) };
      bool res = FirstHitRay(ps.ep1.ie,ps.ep1.r, org,dir, aXY);
      if( !res ) continue;
      int jp0 = ps.ep1.ie;
      int jp1 = jp0+1; if( jp1 == aXY.size()/2 ){ jp1 = 0; }
      ps.ep1.x = (1.0-ps.ep1.r)*aXY[jp0*2+0] + ps.ep1.r*aXY[jp1*2+0];
      ps.ep1.y = (1.0-ps.ep1.r)*aXY[jp0*2+1] + ps.ep1.r*aXY[jp1*2+1];
      ps.ep1.t = 0;
    }
    ps.length = sqrt( (ps.ep0.x-ps.ep1.x)*(ps.ep0.x-ps.ep1.x) + (ps.ep0.y-ps.ep1.y)*(ps.ep0.y-ps.ep1.y) );
    ps.r = rand()/(RAND_MAX+1.0);
...
void CPhysics::AddWeakWection_Slit
(CPlate& plt, // (in,out)
 ////
 const std::vector<double>& aXY,
 const CRigidBody& rb,
 ...
 const double max_stress)
{
  ...
  for(int ilf=0;ilf<plt.alForce.size();ilf++){
    ...
    //qDebug() << "CPhysics::AddWeakWection_Slit() - Warning!! Joint point should be middle";
//    std::cout << ie0 << " " << r0 << std::endl;    std::vector<CPlateSection> aPS;
    std::vector<CPlateSection> aPS;
    ...
    for(int ips=0;ips<aPS.size();ips++){
      ...
      for(int iside=0;iside<2;iside++){
        ...
        double len = sqrt( (ps.ep1.x-ps.ep0.x)*(ps.ep1.x-ps.ep0.x) + (ps.ep1.y-ps.ep0.y)*(ps.ep1.y-ps.ep0.y) );
        double len_med = len;
        double dir_med[2];
        {
          const int ne = (int)aXY.size()/2;
          ...
          int ie1 = ps.ep1.ie;
          int ip0s = ie0;
          int ip0e = ie0+1; if( ip0e == ne ){ ip0e = 0; }
          int ip1s = ie1;
          int ip1e = ie1+1; if( ip1e == ne ){ ip1e = 0; }
          double t0[2] = {aXY[ip0e*2+0]-aXY[ip0s*2+0],aXY[ip0e*2+1]-aXY[ip0s*2+1]};
          double t1[2] = {aXY[ip1e*2+0]-aXY[ip1s*2+0],aXY[ip1e*2+1]-aXY[ip1s*2+1]};
          ...
          if( cos_theta < cos(3.1415*0.25) ) continue; // more than sampling direction interval
        ...
        double stress1 = fabs(ltrq.x*dir_med[0]+ltrq.y*dir_med[1]) / z1;
        ////
        double stress = (stress0>stress1) ? stress0 : stress1;
        //        std::cout << fabs(stress0) << " " << fabs(stress1) << std::endl;
        ps.ltrq = ltrq;
...
void CPhysics::AddWeakSection_Inside
(CPlate& plt, // (in,out)
 ////
 const std::vector<double>& aXY,
 const CRigidBody& rb,
 ...
 const double max_stress)
{
  ...
  std::cout << std::endl;  
  {
    ...
    CVector3D sumF(0,0,0);
    {
      ...
      sumF += lgf;
      std::cout << "sumF: " << sumF.x << " " << sumF.y << " " << sumF.z << std::endl;
    }
    CVector3D sumT(0,0,0);    
    {
      ...
      std::cout << "sumT: " << sumT.x << " " << sumT.y << " " << sumT.z << std::endl;
    }    
  }
   */
  //////
...
//      std::cout << bm1.x << " " << bm1.y << " " << bm1.z << std::endl;
...
void CPhysics::CutSlit()
{
  ...
  for(int irb=0;irb<aRigidBody.size();irb++){
    ...
    for(int ij=0;ij<aJoint.size();ij++){
      ...
      double thickness = (j.irb0==irb) ? aPlate[j.irb1].thickness : aPlate[j.irb0].thickness;
      {
        CSlitData sld;
        sld.eu[0] = ep[2]-ep[0];
        sld.eu[1] = ep[3]-ep[1];
        {
          double len = sqrt(sld.eu[0]*sld.eu[0] + sld.eu[1]*sld.eu[1]);
          sld.eu[0] /= len;
          sld.eu[1] /= len;
        }
        if( arealoop > 0 ){
        ...
        bool res0 = this->FirstHitRay(sld.ie0,sld.r0, sld.ci0,sld.eu,aXY);
        bool res1 = this->FirstHitRay(sld.ie1,sld.r1, sld.ci1,sld.eu,aXY);
        aSlitData.push_back( sld );
      }
    }    
    const int nv = (int)aXY.size()/2;
    std::vector<int> aFlgV0(nv,0); // mark if this vertex is between the slit and will be removed
    for(unsigned int isl=0;isl<aSlitData.size();isl++){
      const CSlitData& sld = aSlitData[isl];
      if( sld.ie0 == sld.ie1 ) continue;
      if( sld.ie0 < sld.ie1 ){
        for(int iv=sld.ie0+1;iv<=sld.ie1;iv++){ aFlgV0[iv] = 1; }
      }
      else{
        for(int iv=sld.ie0+1;iv<nv;iv++){ aFlgV0[iv] = 1; }
        for(int iv=0;iv<=sld.ie1;iv++){ aFlgV0[iv] = 1; }
      }
    }
    ...
    for(;itr!=setPointOnLoop.end();itr++){
      ...
      if( pol.islit == -1 ){
        ...
        double r0 = pol.r;
        double pm[2] = { (1.0-r0)*pi[0] + r0*pj[0], (1.0-r0)*pi[1] + r0*pj[1] };
        if( pol.is_flont ){
...
static void myGlVertex3d(const CVector3D& v){
...
//          ::glColor3d(ps.r,ps.g,ps.b);
          ...
          ::glLineWidth(lw);          
          float color[4]; heatmap(color,(ps.weakness-1.0)/4.0);
          ::glColor3fv(color);
          ::glBegin(GL_LINES);
          myGlVertex3d(q0);
          myGlVertex3d(q1);
      ...
      if( is_draw_section_moment ){
        ...
        for(int ips=0;ips<plt.aPS.size();ips++){
          ...
          CVector3D trq = ltrq.x*plt.t + ltrq.y*plt.b + ltrq.z*plt.n;
          float color[4]; heatmap(color,(ps.weakness-1.0)/4.0);
          ::glColor3fv(color);          
          ::myGlVertex3d(qm);
          ::myGlVertex3d(qm+trq);
  ...
  for(int ij=0;ij<aJoint.size();ij++){
    ...
    /*
     ...
     //      std::cout << ij << " " << i << "  " << ep[0] << " " << ep[1] << " " << ep[2] << " " << ep[3] << std::endl;
     ::glPushMatrix();
...
void CPhysics::DrawEnhancedGL(const double scale_fac)
{
  ...
  for(int irb=0;irb<aRigidBody.size();irb++){
      ...
      for(unsigned int icp=0;icp<rb.aCP.size();icp++){
          ...
          if (!rb.aCForce.empty()) {
              CVector3D q = p + scale_force*rb.aCForce[icp];
              QVector3D each_f = QVector3D(rb.aCForce[icp].x, rb.aCForce[icp].y, rb.aCForce[icp].z);
              max_contact_linear = qMax(float(each_f.length()), max_contact_linear);
  ...
  for(int irb=0;irb<aRigidBody.size();irb++){
    ...
    if( aPlate.size() == aRigidBody.size() ){
      ...
      if( is_draw_section ){
        ...
        for(int ips=0;ips<plt.aPS.size();ips++){
          ...
          QVector3D offset = QVector3D(plt.n.x, plt.n.y, plt.n.z) * plt.thickness;
          GLutils::DrawCylinder(QVector3D(q0.x, q0.y, q0.z) + offset, QVector3D(q1.x, q1.y, q1.z) + offset, small_rad);
          GLutils::DrawCylinder(QVector3D(q0.x, q0.y, q0.z) + offset, QVector3D(q0.x, q0.y, q0.z) - offset, small_rad);
          GLutils::DrawCylinder(QVector3D(q1.x, q1.y, q1.z) + offset, QVector3D(q1.x, q1.y, q1.z) - offset, small_rad);
          GLutils::DrawCylinder(QVector3D(q0.x, q0.y, q0.z) - offset, QVector3D(q1.x, q1.y, q1.z) - offset, small_rad);
      ...
      if( is_draw_section_moment ){
        ...
        for(int ips=0;ips<plt.aPS.size();ips++){
          ...
          heatmap(color,(ps.weakness-1.0)/4.0);
          ::glColor3fv(color);
          //GLutils::DrawArrowFixedLength(QVector3D(qm.x, qm.y, qm.z), QVector3D(qmtrq.x, qmtrq.y, qmtrq.z), 0.25f + 0.75f * ps.weakness);
          //qDebug() << ps.weakness;
          GLutils::DrawArrowFixedLength(QVector3D(qm.x, qm.y, qm.z), QVector3D(qmtrq.x, qmtrq.y, qmtrq.z), (0.25f + 0.75f * qMin(0.25 + (ps.weakness-1.0)/4.0, 1.0)) / scale_fac);
        }
      }
    }
    if( is_draw_skeleton ){
    ...
    for(unsigned int icp=0;icp<rb.aCP.size();icp++){
      ...
      if( is_draw_skeleton ){
          ...
          GLutils::DrawCylinder(QVector3D(cg.x, cg.y, cg.z), QVector3D(p.x, p.y, p.z), small_rad);
      }
  ...
  for(int ij=0;ij<aJoint.size();ij++){
    ...
    if( is_draw_skeleton ){
        glColor3f(0.5f, 0.5f, 0.5f);
        GLutils::DrawCylinder(QVector3D(cg0.x, cg0.y, cg0.z), QVector3D(p0.x, p0.y, p0.z), small_rad);
        glColor3f(0.5f, 0.5f, 0.5f);
        GLutils::DrawCylinder(QVector3D(cg1.x, cg1.y, cg1.z), QVector3D(p1.x, p1.y, p1.z), small_rad);
    }
    ...
    if( is_draw_force ){
        ...
        heatmap(color, t2);
        ::glColor3fv(color);
        //GLutils::DrawCylinderFixedLength(a1, a5, small_rad*0.5f, t2*0.5f);
        //GLutils::DrawCylinderFixedLength(a3, a6, small_rad*0.5f, t2*0.5f);
        GLutils::DrawCylinderFixedLength(a1, a5, small_rad*0.5f, 0.5f / scale_fac);
        GLutils::DrawCylinderFixedLength(a3, a6, small_rad*0.5f, 0.5f / scale_fac);
    }
...
bool CPhysics::GetDrawDeformed()
{
    return is_draw_deformed;
}

bool CPhysics::GetDrawSkeleton()
{
    return is_draw_skeleton;
}

bool CPhysics::GetDrawForce()
{
    return is_draw_force;
}

bool CPhysics::GetDrawSection()
{
    return is_draw_section;
}

bool CPhysics::GetDrawSectionMoment()
{
    return is_draw_section_moment;
}

double CPhysics::GetMaximumStress()
{
    return max_stress;
}

void CPhysics::SetDrawDeformed(const bool b)
{
    is_draw_deformed = b;
}

void CPhysics::SetDrawSkeleton(const bool b)
{
    is_draw_skeleton = b;
}

void CPhysics::SetDrawForce(const bool b)
{
    is_draw_force = b;
}

void CPhysics::SetDrawSection(const bool b)
{
    is_draw_section = b;
}
...
void CPhysics::SetMaximumStress(const double d)
{
    max_stress = d;
}

...
public:
    ...
    std::vector<std::pair<CVector3D, CVector3D> >
        aExForce;  // (position,magnitude) of external forces
    ////
...
public:
    ...
    void AddPlate(const double p[3], const double t[3], const double n[3],
                  const double b[3], const double thickness, const double rho,
                  const std::vector<double>& pts);
    void AddRigidBody(const double centre_of_mass[3], const double mass,
                      const std::vector<double>& contact_points);
    void AddWeightToRigidBody(const double mass_pos[3],
                              const double mass_magnitude[3]);
    void AddJoint(const double position[3], const double jp0[4],
    ...
    // display
    void DrawGL();
    void DrawEnhancedGL(const double scale_fac);
    void DrawFloorGL();
    ...
    void SetMaximumStress(const double d);
...
private:  // static functions
    static void EdEd_Potential(double& energy, CVector3D& dEdu, CVector3D& dEdw,
                               const CVector3D& cg, const CVector3D& u,
                               const double mass, const CVector3D& g);
    static void EdEddE_Exforce(double& energy, CVector3D& dEdu, CVector3D& dEdw,
                               CMatrix3& ddEddu, CMatrix3& ddEddw,
                               CMatrix3& ddEdudw, const CVector3D& cg,
                               const CVector3D& pex, const CVector3D& fex,
                               const CVector3D& u, const CMatrix3& R);
    static void EdEddE_Contact(double& energy, CVector3D& dEdu, CVector3D& dEdw,
                               CMatrix3& ddEddu, CMatrix3& ddEddw,
                               CMatrix3& ddEdudw, const CVector3D& cg,
                               const CVector3D& cp, const CVector3D& u,
                               const CMatrix3& R, const double cont_stiff,
                               const CVector3D& n);
    static void EdEddE_ContactFriction(double& energy, CVector3D& dEdu,
                                       CVector3D& dEdw, CMatrix3& ddEddu,
                                       CMatrix3& ddEddw, CMatrix3& ddEdudw,
                                       const CVector3D& cg, const CVector3D& cp,
                                       const CVector3D& u, const CMatrix3& R,
                                       const double cont_stiff);
    static void EdEddE_Joint(
        double& energy, CVector3D& dEdu0, CVector3D& dEdw0, CVector3D& dEdu1,
        CVector3D& dEdw1, CMatrix3& ddEdu0du0, CMatrix3& ddEdu0dw0,
        CMatrix3& ddEdu0du1, CMatrix3& ddEdu0dw1, CMatrix3& ddEdw0dw0,
        CMatrix3& ddEdw0du1, CMatrix3& ddEdw0dw1, CMatrix3& ddEdu1du1,
        CMatrix3& ddEdu1dw1, CMatrix3& ddEdw1dw1, const double trans_stiff,
        const double rot_stiff, const CVector3D& pj, const CVector3D& cg0,
        const CVector3D& u0, const CMatrix3& R0, const CVector3D& cg1,
        const CVector3D& u1, const CMatrix3& R1);
    static void EdEddE_Total(double& E, Eigen::VectorXd& dE,
                             Eigen::MatrixXd& ddE,
    ...
    static void AddMatrix(Eigen::MatrixXd& M, unsigned int i0, unsigned int j0,
                          const CMatrix3& m, bool isnt_inverse);
    static double GetMinDist_LineSegPoint2D(double& t, const double p[2],
                                            const double s[2],
                                            const double e[2]);
    static void NormalizeVector2D(double v[2]);
    static double TriArea2D(const double v1[2], const double v2[2],
                            const double v3[2]);
    static bool IsHitRay(double& r, const double org[2], const double end[2],
                         const double p0[2], const double p1[2]);
    static bool IsForceInsideHalfLoop2D(const std::vector<double>& aXY,
                                        const CPlate::CLocalForce& lf,
                                        const CPlateSection& ps);
    static CVector3D MomentumTri(double& a, CVector3D lg, double p0[2],
                                 double p1[2], double org[2]);
    static CVector3D MomentumHalfLoop2D(double lx, double ly,
                                        const std::vector<double>& aXY,
                                        const CVector3D& lg,
                                        const CPlateSection& ps);
    static CVector3D MomentumLoop2D(double lx, double ly,
                                    const std::vector<double>& aXY,
                                    const CVector3D& lg);
    static double AreaLoop2D(const std::vector<double>& aXY);
    static bool FirstHitRay(int& ie0, double& r, double org[2], double end[2],
                            const std::vector<double>& aXY);

    static void GetHeights(double& eh, double& hmin, unsigned int nH,
                           const std::vector<double>& aXY, double nx,
                           double ny);

    static void GetPlateSection(std::vector<CPlateSection>& aPS, double eh,
                                double hmin, int nH,
                                const std::vector<double>& aXY, double nx,
                                double ny);

    static void GetPlateSection_Slit(std::vector<CPlateSection>& aPS, int ie0,
                                     const std::vector<double>& aXY,
                                     bool is_loop_cc);

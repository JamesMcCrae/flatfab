...
public:
    ...
    void GetPointsTangentsAlongCurve(const int num, QVector<QVector2D> & points,
                                     QVector<QVector2D> & tangents) const;
    ...
    void GetLineIntersections(const QVector2D & line_p,
                              const QVector2D & line_d,
                              QList<BezierCurvePoint> & intersects);
    void GetLineSegmentIntersections(const QVector2D & line_p1,
                                     const QVector2D & line_p2,
                                     QList<BezierCurvePoint> & intersects);
    ...
    void GetCurvePointCorrespondence(const BezierCurve & other,
                                     int & corresp_offset,
                                     bool & corresp_forward) const;
    void InterpolateCurvePointCorrespondence(const BezierCurve & other,
                                             const int corresp_offset,
                                             const bool corresp_forward,
                                             const float t,
                                             BezierCurve & interp_curve) const;

    float Length() const;
    void Reverse();
    void GetSubCurve(const BezierCurvePoint & b1, const BezierCurvePoint & b2,
                     BezierCurve & sub_curve);

private:
    void SamplePointsForSegment(const int i,
                                QVector<QVector2D> & sample_segment) const;

    void Subdivide(const int i, const float t);
    static void Subdivide(const BezierCurve & c, const int i, const float t,
                          QList<QVector2D> & subdiv_pts);
    static void SamplePoint(const QVector2D & p1, const QVector2D & t1,
                            const QVector2D & t2, const QVector2D & p2,
                            const float t, QVector2D & point);
    static void SampleTangent(const QVector2D & p1, const QVector2D & t1,
                              const QVector2D & t2, const QVector2D & p2,
                              const float t, QVector2D & tangent);

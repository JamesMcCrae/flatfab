...
public:
    ...
    void SetNewBasis(const QVector3D & new_t, const QVector3D & new_n,
                     const QVector3D & new_b, const QVector3D & new_p);
    ...
    void CreateCircle(QVector2D centre, const float radius,
                      int num_sections = 16);
    void CreateRadial(QVector2D centre, const float base_rad,
                      const int num_sectors, const float radii[9]);
    void CopySectorRadially(QVector2D centre, int num_sectors,
                            int points_per_sector);
    void CreateRadialHoles(QVector2D centre, const float base_rad,
                           const int num_sectors, const float radii[9]);
    ...
    void MoveP(const QVector3D & v);  // note: this updates the bezier curve so
                                      // it remains fixed in space
    void Scale(const float x, const float y);
    void Rotate(const float angle_rad);

    void FlipN();  // note: this updates the bezier curve so it remains fixed in
                   // space
    void FlipTB();  // note: this updates the bezier curve so it remains fixed
                    // in space

    // drawing methods
    void Draw();
    void DrawForPicking(const int pickval);
    void DrawInputPolyline();
    void DrawCurve();
    void DrawCurveControlPoints(const float cam_width);
    void DrawCurveControlPointsHandleStyle(const float cam_width,
    ...
    void DrawDeadzone(const QVector3D & slot_start, const QVector3D & slot_end,
                      const float deadzone_radius);
    static void DrawConvexHull(const QList<QVector3D> & hull,
    ...
    static void SaveHeaderToFile(const QList<PlanarSection> & sections,
                                 QTextStream & ofs);
    void SaveToFile(QTextStream & ofs);
    static int LoadHeaderFromFile(QTextStream & ifs);
    void LoadFromFile(QTextStream & ifs);

    static void ComputePacking(const QList<PlanarSection> & sections,
                               const double width_height_ratio,
                               QVector<QVector2D> & pack_pts, QVector2D & bbox);
    static void SaveToSVG(const QList<PlanarSection> & sections,
    ...
    static void SaveToDXF(const QList<PlanarSection> & sections,
                          QTextStream & ofs, const double metres_per_unit,
                          const int dpi, const double width_height_ratio,
                          const bool use_numeric_labels,
                          const QList<QString> & numeric_labels);
    static void DXFWriteLine(QTextStream & ofs, const QVector3D & p1,
                             const QVector3D & p2, const int layer_index,
                             const int colour_index);
    static void DXFWriteText(QTextStream & ofs, const QVector3D & p1,
                             const QString & text, const int layer_index,
                             const int colour_index, const float height);
    static void SaveConnectionsToOBJ(const QList<PlanarSection> & sections,
                                     QTextStream & ofs);

    // intersection and slot related
    void GetSlots(const PlanarSection & other, QList<QVector2D> & my_slots,
                  QList<QList<QVector2D> > & my_slots_rect) const;

    bool GetIntersectionSlotLine(
        const PlanarSection & other, QVector2D & slot_start,
        QVector2D & slot_end) const;  // 1.  do the planes intersect, what axis?
    void GetContourIntersections(const PlanarSection & other,
                                 QList<QVector3D> & isecs,
                                 QList<bool> & isecs_which)
        const;  // 2.  give me points where EITHER plane intersects contour

    bool LinesIntersectContour(const QList<QVector2D> & lines) const;
    void GetBoundingBox2D(QVector2D & min_v, QVector2D & max_v) const;
    void GetBoundingInterval(const QVector2D & direction, float & min_v,
                             float & max_v) const;
    void ComputeContourLineIntersects(const QVector2D & lp,
                                      const QVector2D & ld,
                                      QList<QVector2D> & intersects) const;

    // physical feasibility
    static void ComputeIntersectors(const QList<PlanarSection> & sections,
    ...
    static bool CycleAssemblable(const QList<PlanarSection> & section,
    ...
    bool SketchEditing();
    void SnapSketchToTemplateCut(const QList<QVector3D> & cut_segments,
                                 const float snap_dist_3d);
    ...
    bool RayIntersect(const QVector3D & p0, const QVector3D & dir,
                      QVector3D & intersect);
    bool MouseOutsideDeadzone(const QVector2D & v, const QVector3D & slot_start,
    ...
    void MoveCtrlPointMouseRayIntersect(const QVector2D & v, const bool keep_g1,
                                        const bool equal_lengths = false);

    bool IsCtrlPointSelected();
    int SelectedCtrlPoint();
    void SelectCtrlPoint(const int ctrl_point_index,
                         const int bez_curve_index = 0);
    ...
    static void GetXZPlanePoints(const QList<PlanarSection> & sections,
                                 QList<QVector3D> & plane_pts);
    static void GetXZConvexHull(const QList<PlanarSection> & sections,
                                QList<QVector3D> & hull);
    static bool GetXZPointInsideConvexHull(const QList<QVector3D> & hull,
                                           const QVector3D & p);

    void SplitAlongLine(const QVector2D & split_p, const QVector2D & split_d,
                        QList<PlanarSection> & split_sections);
    bool IsIntersectingSection(const PlanarSection & other) const;

    void GetCurveBetweenSections(const PlanarSection & section1,
                                 const PlanarSection & section2,
                                 BezierCurve & curve);
    void GetCurveAroundSection(const PlanarSection & section,
                               BezierCurve & curve);
    void GetSectionsAlongCurve(const PlanarSection & start_section,
                               const BezierCurve & curve, const int num_frames,
...
private:
    ...
    static void ComputeSlitNumericLabels(

...
public:
    ...
    void ClosestVertex(const QList<QVector3D> & ps, int & ps_index,
                       int & vert_index);
    void AddEdge(const QPair<int, int> & edge, const QVector3D & norm,
                 const QList<QVector3D> & edge_pts, const int edge_plane);
    int GetNumPatches();
    void Clear();
    void Draw() const;
...
private:
    ...
    QVector3D SampleCycleEdgeContour(const int c_ind, const int e_ind,
                                     const float f);

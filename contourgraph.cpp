...
void ContourGraph::Create(const QList <PlanarSection> & sections)
{
    ...
    //there will be a list of plane-plane intersections, these are the contour graph vertices
    //and contours are the edges
    ...
    qDebug() << "Created contour graph with" << verts.size() << "vertices," << edges.size() << "edges, and" << cycles.size() << "cycles.";

}

void ContourGraph::AddVertex(const QVector3D & v)
{
    verts.push_back(v);
}

QVector3D ContourGraph::Vertex(const int index) const
{
    return verts[index];
}

const QList <QVector3D> & ContourGraph::Verts()
{
    return verts;
}
...
void ContourGraph::ClosestVertex(const QList <QVector3D> & ps, int & ps_index, int & vert_index)
{
...
void ContourGraph::AddEdge(const QPair <int, int> & edge, const QVector3D & norm, const QList <QVector3D> & edge_pts, const int edge_plane)
{
...
int ContourGraph::GetNumPatches()
{
    return cycle_edge_points.size();
}
    ...
    for (int i=0; i<cycle_coons_patches.size(); ++i) {       
        ...
        for (int s=1; s<cycle_coons_patches[i].size(); ++s) {           
            ...
            for (int t=1; t<cycle_coons_patches[i][s].size(); ++t) {
                ...
                //if (s > 2 && t > 2 && s < cycle_coons_patches[i].size()-3 && t < cycle_coons_patches[i][s].size()-3) { continue; }
                //if (s > 2 && t > 2) { continue; }
                ...
                const QVector3D & n = QVector3D::crossProduct((p2-p1).normalized(), (p4-p1).normalized());
    ...
    if (draw_cycles) {
        ...
        for (int i=0; i<cycle_edge_points.size(); ++i) {
            ...
            for (int j=0; j<cycle_edge_points[i].size(); ++j) {
                for (int k=0; k<cycle_edge_points[i][j].size(); ++k) {
                    glVertex3f(cycle_edge_points[i][j][k].x(), cycle_edge_points[i][j][k].y(), cycle_edge_points[i][j][k].z());
                }
            }
            glEnd();
        }
        ...
        for (int i=0; i<edges.size(); ++i) {
            ...
            for (int j=0; j<edge_points[i].size(); ++j) {
                glVertex3f(edge_points[i][j].x(), edge_points[i][j].y(), edge_points[i][j].z());
            }
            glEnd();
        }
    ...
    for (int i=0; i<cycle_coons_patches.size(); ++i) {
        ...
        for (int s=1; s<patch_size; ++s) {
            for (int t=1; t<patch_size; ++t) {
                ...
                const int index1 = base_patch_index + (s-1) * patch_size + (t-1);
                const int index2 = base_patch_index + (s) * patch_size + (t-1);
                const int index3 = base_patch_index + (s) * patch_size + (t);
                const int index4 = base_patch_index + (s-1) * patch_size + (t);

                ofs << "f " << index1 << " " << index2 << " " << index3 << " " << index4 << "\n";
...
void ContourGraph::ComputeVerts(const QList <PlanarSection> & sections)
{
    ...
    for (int i=0; i<sections.size(); ++i) {
        ...
        for (int j=i+1; j<sections.size(); ++j) {
            ...
            //we now have to go through each contour, and organize intervals of the contour into EDGES
...
void ContourGraph::ComputeEdges(const QList <PlanarSection> & sections)
{
    ...
    //iterate through the section i's contour, finding all the intersection points, and push them back
    for (int i=0; i<sections.size(); ++i) {
        ...
        for (int j=0; j<contour_verts.size(); ++j) {
            ...
            for (int k=0; k<sections.size(); ++k) {
                ...
                //check for a contour-plane intersection with the current line segment of the contour
                QVector3D intersect;
                if (GLutils::LineSegmentPlaneIntersection(sections[k].P(), sections[k].N(), v1, v2, intersect)) {
        ...
        while (split_index.size() >= 2) {
            ...
            const int start_index = (cur_split_index + 1) % contour_verts.size();
            const int end_index = (split_index.first() + 1) % contour_verts.size();
...
void ContourGraph::ComputeCycles()
{
    ...
    for (int i=0; i<verts_walked.size(); ++i) {
        ...
        //for each spreader, we see the verts we can get to from here, and if we took that edge already
        for (int j=0; j<edges.size(); ++j) {
            ...
            int next_vert = (edges[j].first == end_vert) ? edges[j].second : edges[j].first;

            //new edge index
            const int new_edge_key = j;
            //qDebug() << "at vertex" << end_vert << "considering edge" << new_edge_key << "to go to" << next_vert;
            ...
            if (!edges_walked[i].contains(new_edge_key)) {
                ...
                if (edgeset_not_yet_walked && new_edges_walked.size() <= max_cycle_size) {

                    //qDebug() << "pushing back path" << new_edges_walked; // << edges_walked;
...
void ContourGraph::ComputeCycleEdgePoints(const QList <PlanarSection> & sections)
{
    ...
    for (int i=0; i<cycle_edges.size(); ++i) {
        ...
        for (int j=0; j<each_cycle_edge_normals.size(); ++j) {
            ...
            if (QVector3D::dotProduct(edge_p-sections[cur_edge_plane].P(), each_cycle_edge_normals[j]) < 0.0f) {
        ...
        //qDebug() << "processed cycle" << i << each_cycle_edge_normals.size() << each_cycle_edge_points.size();
    ...
    //for our cycles with just 2 edges, we need to split them both to create a 4-cycle, for a "coons patch formulation"
    for (int i=0; i<cycle_edge_points.size(); ++i) {
...
void ContourGraph::ComputeCycleEdgeArcLengths()
{
    ...
    for (int i=0; i<cycle_edge_points.size(); ++i) {
        ...
        cycle_edge_lengths[i] = QVector <float> (cycle_edge_points[i].size(), 0.0f);
        cycle_edge_arclengths[i].resize(cycle_edge_points[i].size());

        //for each cycle's edge
        for (int j=0; j<cycle_edge_points[i].size(); ++j) {

            cycle_edge_arclengths[i][j] = QVector <float> (cycle_edge_points[i][j].size(), 0.0f);

            //for each cycle's edge's points
            for (int k=1; k<cycle_edge_points[i][j].size(); ++k) {

                //add segment length to edge length
                const float each_len = (cycle_edge_points[i][j][k] - cycle_edge_points[i][j][k-1]).length();
    ...
    qDebug() << cycle_edge_points.size() << cycle_edge_arclengths.size() << "these should be equal";
...
void ContourGraph::PruneCycles(const QList <PlanarSection> & sections)
{
    ...
    for (int i=0; i<cycles.size(); ++i) {
        ...
        //pruning rule 2: remain within union of halfspaces defined by planes taken
        for (int j=0; j<cycle_edges[i].size(); ++j) {

            int edge_plane1 = edge_resting_plane[cycle_edges[i][j]];

            //for all edges not part of edge plane, compute their halfspace side (they should be all negative or all positive)
            ...
            for (int k=0; k<cycle_edges[i].size(); ++k) {
                ...
                QList <QVector3D> & other_edge_pts = edge_points[cycle_edges[i][k]];

                QVector3D edge_midpt = other_edge_pts[other_edge_pts.size()/2];

                const float each_dot_prod = QVector3D::dotProduct(sections[edge_plane1].N(), edge_midpt - sections[edge_plane1].P());
...
QVector3D ContourGraph::SampleCycleEdgeContour(const int c_ind, const int e_ind, const float f)
{
    ...
    qDebug() << "ContourGraph::SampleCycleEdgeContour - Warning, didn't find suitable point along cycle" << c_ind << "edge" << e_ind << "given parameter" << f;
    ...
    //for (int i=0; i<1; ++i) {
        ...
        for (int s=0; s<=patch_samples; ++s) {
            ...
            for (int t=0; t<=patch_samples; ++t) {
                ...
                const QVector3D C_s0 = SampleCycleEdgeContour(i, 0, s_f); //1st
                const QVector3D D_1t = SampleCycleEdgeContour(i, 1, t_f); //2nd
                const QVector3D C_s1 = SampleCycleEdgeContour(i, 2, 1.0f - s_f); //3rd - flip
                const QVector3D D_0t = SampleCycleEdgeContour(i, 3, 1.0f - t_f); //4th - flip
                ...
                if (do_hermite) {
                    ...
                    //const QVector3D N_0t = (cycle_edge_normals[i][0]) * norm_factor;
                    //const QVector3D N_1t = (-cycle_edge_normals[i][2]) * norm_factor;
                    //const QVector3D N_s0 = (cycle_edge_normals[i][3]) * norm_factor;
                    //const QVector3D N_s1 = (-cycle_edge_normals[i][1]) * norm_factor;
                    ...
                    //const QVector3D H_c = C_s0 * coeff_t[0] + N_s0 * coeff_t[1] + C_s1 * coeff_t[2] + N_s1 * coeff_t[3];
                    //const QVector3D H_d = D_0t * coeff_s[0] + N_0t * coeff_s[1] + D_1t * coeff_s[2] + N_1t * coeff_s[3];
                    //const QVector3D H_c = C_s0 * coeff_t[0] + N_s0 * coeff_t[1] * (1.0f - s_f) + C_s1 * coeff_t[2] + N_s1 * coeff_t[3] * s_f;
                    //const QVector3D H_d = D_0t * coeff_s[0] + N_0t * coeff_s[1] * (1.0f - t_f) + D_1t * coeff_s[2] + N_1t * coeff_s[3] * t_f;

                    //this one is off in multiple directions
                    //const QVector3D H_c = c0_s * coeff_t[0] + N0_s * coeff_t[1] + c1_s * coeff_t[2] + N1_s * coeff_t[3];
                    //const QVector3D H_d = d0_t * coeff_s[0] + N0_t * coeff_s[1] + d1_t * coeff_s[2] + N1_t * coeff_s[3];
                    ...
                    //e.g. for curve defined by P(s,0), it's pushed by the normal N0_s
                    //const QVector3D H_c = c0_s * coeff_t[0] + N0_s * coeff_t[1]* coeff_s[0] * s_f + c1_s * coeff_t[2] + N1_s * coeff_t[3]* coeff_s[2] * (1.0f - s_f);
                    //const QVector3D H_d = d0_t * coeff_s[0] + N0_t * coeff_s[1]* coeff_t[0] * t_f + d1_t * coeff_s[2] + N1_t * coeff_s[3]* coeff_t[2] * (1.0f - t_f);

                    //const QVector3D H_c = c0_s * coeff_t[0] + N0_s * (coeff_t[1]* coeff_s[0] - coeff_t[3]* coeff_s[2]) + c1_s * coeff_t[2] + N1_s * (coeff_t[3]* coeff_s[2] - coeff_t[1]* coeff_s[0]);
                    //const QVector3D H_d = d0_t * coeff_s[0] + N0_t * (coeff_s[1]* coeff_t[0] - coeff_s[3]* coeff_t[2]) + d1_t * coeff_s[2] + N1_t * (coeff_s[3]* coeff_t[2] - coeff_s[1]* coeff_t[0]);

                    //this B needs to contain some "normal killing" terms, otherwise it is working
                    //const QVector3D B = c0_0 * (1.0f - s_f) * (1.0f - t_f) + c0_1 * s_f * (1.0f - t_f) + c1_1 * s_f * t_f + c1_0 * (1.0f - s_f) * t_f;
                    //forget the normals, the curves intersect the boundary, we need correct basis functions
                    //const QVector3D B = c0_0 * (1.0f - s_f) * (1.0f - t_f) + c0_1 * s_f * (1.0f - t_f) + c1_1 * s_f * t_f + c1_0 * (1.0f - s_f) * t_f
                    //        + N0_t * coeff_t[0] * coeff_s[1] + N1_t * coeff_t[2] * coeff_s[3] + N0_s * coeff_s[0] * coeff_t[1] + N1_s * coeff_s[2] * coeff_t[3];
                    ...
                    const QVector3D N0_s = cycle_edge_normals[i][0] * norm_factor;
                    const QVector3D N1_s = (-cycle_edge_normals[i][2]) * norm_factor;
                    const QVector3D N0_t = (-cycle_edge_normals[i][3]) * norm_factor;
                    const QVector3D N1_t = cycle_edge_normals[i][1] * norm_factor;

                    const QVector3D row1 = coeff_t[0] * (c0_s * coeff_s[0] + c1_s * coeff_s[2] + N0_s * coeff_s[1] + N1_s * coeff_s[3]);
                    const QVector3D row2 = coeff_t[2] * (d0_t * coeff_s[0] + d1_t * coeff_s[2] + N0_t * coeff_s[1] + N1_t * coeff_s[3]);

                    cycle_coons_patches[i][s][t] = row1;// + row2;
                    */

                    //qDebug() << s_f << c0_s << c1_s << cycle_edge_normals[i][0] << cycle_edge_normals[i][1] << cycle_edge_normals[i][2] << cycle_edge_normals[i][3];
                    ...
                    const QVector3D B = C_00 * (1.0f - s_f) * (1.0f - t_f) + C_01 * s_f * (1.0f - t_f) + C_11 * s_f * t_f + C_10 * (1.0f - s_f) * t_f;
